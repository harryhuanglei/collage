 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: adsense.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'download')
{
    ...
    $sql = "SELECT a.ad_id, a.ad_name, b.* ".
           "FROM " .$hhs->table('ad'). " AS a, " .$hhs->table('adsense'). " AS b ".
    ...
    while ($rows = $db->fetchRow($res))
    {
        ...
        $sql2 = 'SELECT COUNT(order_id) FROM ' .$hhs->table('order_info'). " WHERE from_ad='$rows[ad_id]' AND referer='$rows[referer]'";
        $rows['order_num'] = $db->getOne($sql2);

        /* 当前广告所产生的已完成的有效订单 */
        $sql3 = "SELECT COUNT(order_id) FROM " .$hhs->table('order_info').
    ...
    /* 站外JS投放商品的统计数据 */
    $goods_stats    = array();
    $goods_sql      = "SELECT from_ad, referer, clicks FROM " .$hhs->table('adsense').
    ...
    while ($rows2 = $db->fetchRow($goods_res))
    {
        ...
        $rows2['order_num'] = $db->getOne("SELECT COUNT(order_id) FROM " .$hhs->table('order_info'). " WHERE referer='$rows2[referer]'");

        /* 当前广告所产生的已完成的有效订单 */

        $sql = "SELECT COUNT(order_id) FROM " .$hhs->table('order_info').
