...

{
    ...
    $sql = "INSERT INTO ".$hhs->table('bonus_type')." (free_all,is_online,suppliers_id,only_first,tongyong,number,is_share,type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount)

    VALUES ('$_POST[free_all]','$is_online','$_POST[suppliers_id]','$_POST[only_first]','$_POST[tongyong]','$_POST[number]','$_POST[is_share]','$type_name',
...

{
    ...
    $suppliers_id = $db->getOne("select `suppliers_id` FROM ".$hhs->table('bonus_type')." WHERE `type_id` = '".$type_id."' ");
...

{
    ...
    $suppliers_id = $db->getOne("SELECT `suppliers_id` FROM ".$hhs->table('bonus_type')." WHERE `type_id` = " . $bonus_typeid);
...

{
...

    {
		...
		$filter['suppliers_id'] = ($_REQUEST['suppliers_id'] == '') ? '' : trim($_REQUEST['suppliers_id']);
		
		$filter['type_name'] = ($_REQUEST['type_name'] == '') ? '' : trim($_REQUEST['type_name']);
...

		{
			...
			$where = " where type_name like '%$filter[type_name]%'";
...

{
    ...
    $sql = "SELECT ub.*, u.uname as 'user_name', u.email, o.order_sn, bt.type_name ".
