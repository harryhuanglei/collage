...
elseif ($_REQUEST['act'] == 'get_article_list') {
	...
	$sql = 'SELECT article_id, title FROM ' .$hhs->table('article'). $where. 'ORDER BY article_id DESC LIMIT 50';
...
function get_keywords_articles($kws_id,$db) {
	...
	if(!empty($articles)){
		...
		$sql2 = "SELECT `article_id`,`title` FROM ".$GLOBALS['hhs']->table('article')." WHERE `article_id` IN ($articles)";
