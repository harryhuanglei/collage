 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: database.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
{
    $tables = $db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($hhs->prefix) . "%'");
...
if ($_REQUEST['act'] == 'restore11')
{
    ...
    else
    {
        ...
        foreach ($real_list AS $file)
        {
            ...
            $list[] = array('name' => $file, 'ver' => $info['hhs_ver'], 'add_time' => $info['date'], 'vol' => $info['vol'], 'file_size' => num_bitunit($file_size), 'mark' => $mark);
        }
    }
...
if ($_REQUEST['act'] == 'dumpsql')
{
    ...
    {
        case 'full':
            $except = array($hhs->prefix.'sessions', $hhs->prefix.'sessions_data');
            $temp = $db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($hhs->prefix) . "%'");
...
if ($_REQUEST['act'] == 'import')
{
    ...
    if (preg_match('/_[0-9]+\.sql$/', $file_name))
    {
        ...
        {
            $info = cls_sql_dump::get_head($path . $file);
            if ($info['hhs_ver'] != VERSION )
            {
                sys_msg(sprintf($_LANG['version_error'], VERSION, $sql_info['hhs_ver']));
            }
...
if ($_REQUEST['act'] == 'upload_sql')
{
    ...
    if (empty($sql_info['hhs_ver']))
    {
        sys_msg($_LANG['unrecognize_version'], 1);
    }
    else
    {
        if ($sql_info['hhs_ver']!= VERSION)
        {
            sys_msg(sprintf($_LANG['version_error'], VERSION, $sql_info['hhs_ver']));
        }
    }
...
if ($_REQUEST['act'] == 'optimize')
{
    ...
    $db_ver = $db_ver_arr;
    $ret = $db ->query("SHOW TABLE STATUS LIKE '" . mysql_like_quote($hhs->prefix) . "%'");
