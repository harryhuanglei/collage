 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: goods_type.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
$exc = new exchange($hhs->table("goods_type"), $db, 'cat_id', 'cat_name');
...
if ($_REQUEST['act'] == 'manage')
{
    ...
    $query = $db->query("SELECT a.cat_id FROM " . $hhs->table('attribute') . " AS a RIGHT JOIN " . $hhs->table('goods_attr') . " AS g ON g.attr_id = a.attr_id GROUP BY a.cat_id");
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($db->autoExecute($hhs->table('goods_type'), $goods_type) !== false)
    {
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($db->autoExecute($hhs->table('goods_type'), $goods_type, 'UPDATE', "cat_id='$cat_id'") !== false)
    {
        /* 对比原来的分组 */
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->drop($id))
    {
        ...
        /* 清除该类型下的所有属性 */
        $sql = "SELECT attr_id FROM " .$hhs->table('attribute'). " WHERE cat_id = '$id'";
        $arr = $db->getCol($sql);

        $GLOBALS['db']->query("DELETE FROM " .$hhs->table('attribute'). " WHERE attr_id " . db_create_in($arr));
        $GLOBALS['db']->query("DELETE FROM " .$hhs->table('goods_attr'). " WHERE attr_id " . db_create_in($arr));
...
function get_goodstype()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['hhs']->table('goods_type');
        ...
        $sql = "SELECT t.*, COUNT(a.cat_id) AS attr_count ".
               "FROM ". $GLOBALS['hhs']->table('goods_type'). " AS t ".
               "LEFT JOIN ". $GLOBALS['hhs']->table('attribute'). " AS a ON a.cat_id=t.cat_id ".
               "GROUP BY t.cat_id " .
...
{
    $sql = "SELECT * FROM " .$GLOBALS['hhs']->table('goods_type'). " WHERE cat_id='$cat_id'";
...
{
    $sql = "UPDATE " . $GLOBALS['hhs']->table('attribute') .
