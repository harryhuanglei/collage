 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: ads.php 17217 2014-05-12 06:29:08Z pangbin $
*/
define('IN_HHS', true);
require(dirname(__FILE__) . '/includes/init.php');
include_once(ROOT_PATH . 'includes/cls_image.php');
$image = new cls_image($_CFG['bgcolor']);
$exc   = new exchange($hhs->table("ad"), $db, 'ad_id', 'ad_name');
/* act操作项的初始化 */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = 'SELECT * FROM ' .$GLOBALS['hhs']->table('ad_position'). ' ORDER BY position_id DESC';
	$smarty->assign('ad_position_list',$db->getAll($sql));
...
elseif ($_REQUEST['act'] == 'add')
{
	...
	$site_list = get_site_lists();
	$smarty->assign('site_list',       $site_list);
    $smarty->assign('form_act', 'insert');
    $smarty->assign('action',   'add');
    $smarty->assign('cfg_lang', $_CFG['lang']);
    $smarty->assign('floor_list',       getCateFloorInfo());
    assign_query_info();
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 查看广告名称是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$hhs->table('ad'). " WHERE ad_name = '$ad_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据 */
    $sql = "INSERT INTO ".$hhs->table('ad'). " (cate_id,app_type,app_type_id,ad_width,city_id,position_id,media_type,ad_name,ad_link,ad_code,start_time,end_time,link_man,link_email,link_phone,click_count,enabled)
    VALUES (
            '$_POST[cate_id]',
			'$_POST[app_type]',
			'$_POST[app_type_id]',
...
elseif ($_REQUEST['act'] == 'edit')
{
    admin_priv('ad_add');
    /* 获取广告数据 */
    $sql = "SELECT * FROM " .$hhs->table('ad'). " WHERE ad_id='".intval($_REQUEST['id'])."'";
...
elseif ($_REQUEST['act'] == 'add_js')
{
    ...
    $js_code .= ' src='.'"'.$hhs->url().'affiche.php?act=js&type='.$_REQUEST['type'].'&ad_id='.intval($_REQUEST['id']).'"'.'></script>';
    $site_url = $hhs->url().'affiche.php?act=js&type='.$_REQUEST['type'].'&ad_id='.intval($_REQUEST['id']);
...
function get_adslist()
{
    ...
    $sql = 'SELECT ad.*, COUNT(o.order_id) AS ad_stats, p.position_name '.
            'FROM ' .$GLOBALS['hhs']->table('ad'). 'AS ad ' .
            'LEFT JOIN ' . $GLOBALS['hhs']->table('ad_position'). ' AS p ON p.position_id = ad.position_id '.
            'LEFT JOIN ' . $GLOBALS['hhs']->table('order_info'). " AS o ON o.from_ad = ad.ad_id $where " .
            'GROUP BY ad.ad_id '.
...
//获取添加的站点列表
function get_site_lists()
{
	$cities_id_list = $GLOBALS['db']->getAll('SELECT city_id FROM ' .$GLOBALS['hhs']->table('site'));
	foreach ($cities_id_list as $key => $value)
	{
		$cities_id_list[$key]['region_name'] =get_region_name($value['city_id']);
	}
	return $cities_id_list;
}
/*获取楼层信息即分类名称*/
function getCateFloorInfo()
{
    $sql = 'SELECT cat_id,cat_name  FROM ' . $GLOBALS['hhs']->table('category') ." WHERE parent_id = 0 AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
