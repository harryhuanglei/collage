                    ...
                    if(!empty($province)&&!empty($city)&&!empty($district))
                    {
                        ...
                        if(!empty($invoice_no))
                        {
                            ...
                            if($operable_list[split])
                            {
                                ...
                                /*APP推送*/
                                $user = $db->getRow('SELECT devicetoken FROM '.$hhs->table('users').' WHERE user_id = ' . $order['user_id']);
                                ...
                                if ($tokens)
                                {
                                    ...
                                    if(strlen($tokens) == 44)
                                    {
                                        ...
                                        $result =  $umeng ->sendAndroidUnicast($tokens,'发货提醒','您的订单已经发货啦！','请进入APP查看物流详情','ucenter','','go_url');
                                    }
                                    elseif(strlen($tokens) == 64)
                                    {
                                        /*苹果*/
                                        $umeng = new Umeng($_CFG['umeng_ios_key'],$_CFG['umeng_ios_secret']);
                                        $result =  $umeng ->sendIOSUnicast($tokens,'发货提醒','ucenter');
                                    }
                                }
                            }
                        }
...
//立即成团
elseif('team_success' == $_REQUEST['act'])
{
	$team_sign = intval($_REQUEST['team_sign']);
    $sql="select * from ".$hhs->table('order_info')." where team_sign=".$team_sign." and team_status = 1 and pay_status = 2";
    $team_order=$db->getAll($sql);
	/*指定团标志支付的订单的个数*/
	$sql="select count(*) from ".$hhs->table('order_info')." where team_sign=".$team_sign." and team_status = 1 and pay_status = 2";
	/*立即成团时团数量重新修正，舞动舞动   死道友不死频道*/
    $team_num=$db->getOne($sql);
    foreach($team_order as $v)
	{
        $arr['team_status']  = 2;
        $arr['teammen_num']  = $team_num;
		$arr['team_num'] = $team_num;
        update_order($v['order_id'], $arr);
        $user_id=$v['user_id'];
	...
	$sql="select * from ".$hhs->table('order_info')." where team_sign=".$team_sign." and pay_status < 2";
    $team_order=$db->getAll($sql);
	foreach($team_order as $v)
	{
		$arr_x['order_status']  = 2;
        $arr_x['shipping_status']  = 0;
		$arr_x['pay_status']  = 0;
        update_order($v['order_id'], $arr_x);
	}
	$sql="select * from ".$hhs->table('order_info')." where team_sign=".$team_sign." and pay_status > 2";
    $team_order=$db->getAll($sql);
	foreach($team_order as $v)
	{
		$arr_y['team_status']  = 2;
        update_order($v['order_id'], $arr_y);
	}
	$links[] = array('text' => '成团成功', 'href' => 'order.php?act=team_list');
    sys_msg("成功处理", 1, $links);
}
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    /* 团长优惠*/
    if($order['team_status']==2&&$order['team_first']==1)
    {//团长团购成功才计算优惠
...

            {
                ...
                order_action($order['order_sn'], 1, 0, 2, $action_note);
                 ...
                 //发送邮件
                  ...
                  $row = $db->getRow('SELECT openid,uname FROM '.$hhs->table('users').' WHERE user_id = ' . $order['user_id']);
...
function order_list($refund_ex="",$is_page=true,$is_suppliers=false)
{
    ...
    if ($result === false)
    {
       ...
       $filter['end_time'] = empty($_REQUEST['end_time']) ? '' : (strpos($_REQUEST['end_time'], '-') > 0 ?  local_strtotime($_REQUEST['end_time']) : $_REQUEST['end_time']);
		...
		$filter['is_app_buy'] = isset($_REQUEST['is_app_buy']) ? intval($_REQUEST['is_app_buy']) : '';
       ...
       //and extension_code!='team_goods'
		...
		if($filter['is_app_buy']!='')
		{
			$where .= " AND o.is_app_buy = '$filter[is_app_buy]'";
		}
		...
		if(!$is_suppliers)
		{
	        ...
	        if ($filter['city_id'])
	        {
	            $where .= " AND o.city = '$filter[city_id]'";
	        }
	        if ($filter['district_id'])
	        {
	            $where .= " AND o.district = '$filter[district_id]'";
	        }
		}
        ...
        if($is_suppliers){
            ...
            if ($filter['city_id'])
	        {
	            $where .= " AND sp.city = '$filter[city_id]'";
	        }
	 		if ($filter['district_id'])
	        {
	            $where .= " AND sp.district = '$filter[district_id]'";
	        }
        }
		else
		{
        ...
        else
        {
        	...
        	" LEFT JOIN " .$GLOBALS['hhs']->table('goods'). " AS g ON o.extension_id=g.goods_id ".
        	" LEFT JOIN " .$GLOBALS['hhs']->table('shipping_point'). " AS sp ON o.point_id=sp.id ".
                    $where;
        }
        ...
        /* 查询 */
        if($is_page){
            $sql = "SELECT o.suppliers_id,o.is_app_buy,o.point_shop_remind,o.checked_mobile,o.best_time,o.order_type,o.surplus,o.bonus,o.postscript, o.package_one, o.point_id,o.transaction_id,o.team_status,o.team_sign,o.team_first, o.order_id, o.order_sn, o.add_time,o.pay_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
    ...
    $row[$key]['refund_goods_list'] = get_order_goods_list($value['order_id'], " and refund_status>0");
        ...
        $row[$key]['suppliers_name']= $value['suppliers_id'] > 0 ? get_suppliers_name($value['suppliers_id']) : '自营店';
    }
