 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: attribute.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
$exc = new exchange($hhs->table("attribute"), $db, 'attr_id', 'attr_name');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    {
        $sql = "SELECT * FROM " . $hhs->table('attribute') . " WHERE attr_id = '$_REQUEST[attr_id]'";
        $attr = $db->getRow($sql);
    }
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        $db->autoExecute($hhs->table('attribute'), $attr, 'INSERT');
    ...
    {
        $db->autoExecute($hhs->table('attribute'), $attr, 'UPDATE', "attr_id = '$_POST[attr_id]'");
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        $sql = "DELETE FROM " . $hhs->table('attribute') . " WHERE attr_id " . db_create_in($ids);
        $db->query($sql);

        $sql = "DELETE FROM " . $hhs->table('goods_attr') . " WHERE attr_id " . db_create_in($ids);
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $db->query("DELETE FROM " .$hhs->table('attribute'). " WHERE attr_id='$id'");
    $db->query("DELETE FROM " .$hhs->table('goods_attr'). " WHERE attr_id='$id'");
...
elseif ($_REQUEST['act'] == 'get_attr_num')
{
    ...
    $sql = "SELECT COUNT(*) ".
           " FROM " . $hhs->table('goods_attr') . " AS a, ".
...
function get_attrlist()
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('attribute') . " AS a $where";
    ...
    $sql = "SELECT a.*, t.cat_name " .
            ...
            " LEFT JOIN " . $GLOBALS['hhs']->table('goods_type') . " AS t ON a.cat_id = t.cat_id " . $where .
