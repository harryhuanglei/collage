...
$exc = new exchange($hhs->table("brand"), $db, 'brand_id', 'brand_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$hhs->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order) ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, brand_desc, brand_logo, is_show, sort_order ".
            "FROM " .$hhs->table('brand'). " WHERE brand_id='$_REQUEST[id]'";
    $brand = $db->GetRow($sql);
...
elseif($_REQUEST['act'] == 'add_brand')
{
    ...
    {
        $sql = "INSERT INTO " . $hhs->table('brand') . "(brand_name)" .
               "VALUES ( '$brand')";
...
{
    /*check_authz_json('brand_remove');*/

    $id = intval($_GET['id']);

    /* 删除该品牌的图标 */
    $sql = "SELECT brand_logo FROM " .$hhs->table('brand'). " WHERE brand_id = '$id'";
    ...
    /* 更新商品的品牌编号 */
    $sql = "UPDATE " .$hhs->table('goods'). " SET brand_id=0 WHERE brand_id='$id'";
    $db->query($sql);
...
{
    /* 权限判断 */
    /*admin_priv('brand_remove');*/
    $brand_id = isset($_GET['id']) ? intval($_GET['id']) : 0;

    /* 取得logo名称 */
    $sql = "SELECT brand_logo FROM " .$hhs->table('brand'). " WHERE brand_id = '$brand_id'";
    $logo_name = $db->getOne($sql);

    if (!empty($logo_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/brandlogo/' .$logo_name);
        $sql = "UPDATE " .$hhs->table('brand'). " SET brand_logo = '' WHERE brand_id = '$brand_id'";
        $db->query($sql);
    }
...
function get_brandlist()
{
    ...
    if ($result === false)
    {
        ...
        {
            $sql = "SELECT COUNT(*) FROM ".$GLOBALS['hhs']->table('brand') .' WHERE brand_name = \''.$_POST['brand_name'].'\'';
        }
        else
        {
        ...
        if (isset($_POST['brand_name']))
        {
            ...
            $sql = "SELECT * FROM ".$GLOBALS['hhs']->table('brand')." WHERE brand_name like '%{$keyword}%' ORDER BY sort_order ASC";
        }
        else
        {
            $sql = "SELECT * FROM ".$GLOBALS['hhs']->table('brand')." ORDER BY sort_order ASC";
        }
