 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: user_rank.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
$exc = new exchange($hhs->table("user_rank"), $db, 'rank_id', 'rank_name');
$exc_user = new exchange($hhs->table("users"), $db, 'user_rank', 'user_rank');
...
{
    $ranks = array();
    $ranks = $db->getAll("SELECT * FROM " .$hhs->table('user_rank'));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO " .$hhs->table('user_rank') ."( ".
...
elseif ($_REQUEST['act'] == 'edit_min_points')
{
    ...
    $rank = $db->getRow("SELECT max_points, special_rank FROM " . $hhs->table('user_rank') . " WHERE rank_id = '$rank_id'");
...
elseif ($_REQUEST['act'] == 'edit_max_points')
{
    ...
    $rank = $db->getRow("SELECT min_points, special_rank FROM " . $hhs->table('user_rank') . " WHERE rank_id = '$rank_id'");
