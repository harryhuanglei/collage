...

{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price, v.attr_team_price, v.attr_img ".
...

{
...

    {
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            ...
            '属性团购价 <input type="text" name="attr_team_price_list[]" value="' . $val['attr_team_price'] . '" size="5" maxlength="10" />' :
...

{
...

    {
		...
		$filter['goods_sale_type']   = empty($_REQUEST['goods_sale_type']) ? '' : trim($_REQUEST['goods_sale_type']);
        ...
        if ($filter['goods_sale_type']) {

            $where .= " AND $filter[goods_sale_type]=1";
        ...
        if(!$filter['suppliers_id']&&$is_delete==0)
        ...
        $sql = "SELECT is_app,is_luck,is_miao,is_fresh,is_tejia,is_mall,is_zero,is_team,team_num,team_price,limit_buy_one,goods_id,sales_num, goods_name, goods_type, goods_sn, shop_price, is_on_sale,is_check, is_best, is_new, is_hot, sort_order, goods_number, integral, " .

                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".

                    ",suppliers_id FROM " . $GLOBALS['hhs']->table('goods') . " AS g WHERE is_delete='$is_delete' $where" .
    ...
    { 
        ...
        $row[$id]['suppliers_name'] = get_suppliers_name($v['suppliers_id']);
