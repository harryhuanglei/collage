 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: cls_sql_dump.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
class cls_sql_dump
{
    ...
    function dump_table($path, $vol)
    {
        ...
        $this->dump_sql .= '-- END hhshop v2.x SQL Dump Program ';
    ...
    {
        /* 获取sql文件头部信息 */
        $sql_info = array('date'=>'', 'mysql_ver'=> '', 'php_ver'=>0, 'hhs_ver'=>'', 'vol'=>0);
        $fp = fopen($path,'rb');
        $str = fread($fp, 250);
        fclose($fp);
        $arr = explode("\n", $str);
        ...
        foreach ($arr AS $val)
        {
            ...
            if ($pos > 0)
            {
                ...
                elseif ($type == 'HHShop VERSION')
                {
                    $sql_info['hhs_ver'] = $value;
                }
                elseif ($type == 'Vol')
                {
                    $sql_info['vol'] = $value;
