...
function admin_log($sn = '', $action, $content)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['hhs']->table('admin_log') . ' (log_time, user_id, log_info, ip_address) ' .
...
function insert_config($parent, $code, $value)
{
    ...
    $sql = 'SELECT id FROM ' . $hhs->table('shop_config') . " WHERE code = '$parent' AND type = 1";
    $parent_id = $db->getOne($sql);

    $sql = 'INSERT INTO ' . $hhs->table('shop_config') . ' (parent_id, code, value) ' .
...
{
    $rank_list = array();
    $sql = 'SELECT rank_id, rank_name, min_points FROM ' . $GLOBALS['hhs']->table('user_rank');
    if ($is_special)
    {
...
{
    $user_list = array();
    $sql = 'SELECT user_id, user_name FROM ' . $GLOBALS['hhs']->table('users') . $where.
...
function get_position_list()
{
    ...
    $sql = 'SELECT position_id, position_name, ad_width, ad_height '.
           'FROM ' . $GLOBALS['hhs']->table('ad_position');
    $res = $GLOBALS['db']->query($sql);
...
function get_topic_goods_list($filter)
{
    $filter->keyword = json_str_iconv($filter->keyword);
    $where = get_where_sql($filter); // 取得过滤条件
    ...
    /* 取得数据 */
    $sql = 'SELECT goods_id, goods_name, shop_price '.
           'FROM ' . $GLOBALS['hhs']->table('goods') . ' AS g ' . $where . ' and is_mall = 1'.
           ' LIMIT 50';
    $row = $GLOBALS['db']->getAll($sql);
...
function get_article_list($filter)
{
    ...
    $sql = 'SELECT a.article_id, a.title '.
           'FROM ' .$GLOBALS['hhs']->table('article'). ' AS a, ' .$GLOBALS['hhs']->table('article_cat'). ' AS c ' . $where;
    $res = $GLOBALS['db']->query($sql);
...
function get_where_sql($filter)
{
    ...
    $where .= isset($filter->is_team) ? ' AND is_team  = '. $filter->is_team." "  : '';
    $where .= isset($filter->is_zero) ? ' AND is_zero  = '. $filter->is_zero." "  : '';
    $where .= isset($filter->is_miao) ? ' AND is_miao  = '. $filter->is_miao." "  : '';
    $where .= isset($filter->is_luck) ? ' AND is_luck  = '. $filter->is_luck." "  : '';
    $where .= isset($filter->limit_buy_bumber) ? ' AND limit_buy_bumber  = '. $filter->limit_buy_bumber." "  : '';
...
function area_list($region_id)
{
    ...
    $sql = 'SELECT * FROM ' . $GLOBALS['hhs']->table('region').
...
function get_pay_ids()
{
    ...
    $sql = 'SELECT pay_id, is_cod FROM ' .$GLOBALS['hhs']->table('payment'). ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);
...
{
    $sql = 'TRUNCATE TABLE ' .$GLOBALS['hhs']->table($table_name);
...
{
    $sql = "SELECT attr_group FROM " . $GLOBALS['hhs']->table('goods_type') . " WHERE cat_id='$cat_id'";
...
function set_filter($filter, $sql, $param_str = '')
{
    ...
    setcookie('HHSCP[lastfilterfile]', sprintf('%X', crc32($filterfile)), time() + 600);
    setcookie('HHSCP[lastfilter]',     urlencode(serialize($filter)), time() + 600);
    setcookie('HHSCP[lastfiltersql]',  base64_encode($sql), time() + 600);
}
...
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['hhs']->table('brand').
...
function admin_info()
{
    $sql = "SELECT * FROM ". $GLOBALS['hhs']->table('admin_user')."
