...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link2', array('href' => 'statistics.php?act=download', 'text' => "导出"));
	$smarty->assign('point_list',getOrderPoint());
    ...
    $smarty->assign('suppliers_list',suppliers_list_info(' is_check = 1 '));
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('statistics_order_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $tpl_file = 'statistics_order_list.htm';
    $smarty->assign('totle_money',   $order_list['totle_money']);
    $smarty->assign('order_list',   $order_list['orders']);
    $smarty->assign('filter',       $order_list['filter']);
    $smarty->assign('record_count', $order_list['record_count']);
...
function order_list($refund_ex="",$is_page=true,$is_suppliers=false)
{
    ...
    if ($result === false)
    {
      ...
      // var_dump($filter['pay_id']);die;
        if ($filter['pay_id'] != -1)
        {
            $where .= " AND o.pay_id  = '$filter[pay_id]' ";
        }else
		{
			$where .= " AND o.pay_id  > 0 ";
		}
        ...
        //     " LEFT JOIN " .$GLOBALS['hhs']->table('shipping_point_user'). " AS p ON p.openid=u.openid ".
        //     " LEFT JOIN " .$GLOBALS['hhs']->table('goods'). " AS g ON o.extension_id=g.goods_id ".$where;
