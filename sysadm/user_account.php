 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: user_account.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 获得支付方式列表 */
    // $payment = array();
    // $sql = "SELECT pay_id, pay_name FROM ".$hhs->table('payment').
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    {
        /* 取得余额信息 */
        $user_account = $db->getRow("SELECT * FROM " .$hhs->table('user_account') . " WHERE id = '$id'");

        // 如果是负数，去掉前面的符号
        $user_account['amount'] = str_replace('-', '', $user_account['amount']);

        /* 取得会员名称 */
        $sql = "SELECT user_name FROM " .$hhs->table('users'). " WHERE user_id = '$user_account[user_id]'";
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $user_id = $db->getOne("SELECT user_id FROM " .$hhs->table('users'). " WHERE user_name = '$user_name'");
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $sql = "INSERT INTO " .$hhs->table('user_account').
    ...
    {
        /* 更新数据表 */
        $sql = "UPDATE " .$hhs->table('user_account'). " SET ".
    ...
    if ($process_type == 0 && $is_paid == 0)
    {
        ...
        /* 取支付方式信息 */
        $payment_info = array();
        $payment_info = $db->getRow('SELECT * FROM ' . $hhs->table('payment').
...
elseif ($_REQUEST['act'] == 'check')
{
    ...
    if ($id == 0)
    {
        hhs_header("Location: user_account.php?act=list\n");
        exit;
    }

    /* 查询当前的预付款信息 */
    $account = array();
    $account = $db->getRow("SELECT * FROM " .$hhs->table('user_account'). " WHERE id = '$id'");
    ...
    $sql = "SELECT user_name FROM " .$hhs->table('users'). " WHERE user_id = '$account[user_id]'";
    $user_name = $db->getOne($sql);
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    if ($account['is_paid'] == 0)
    {
        ...
        if ($is_paid == '1' && $account['process_type'] == '1')
        {
            ...
            //红包提现
            if($_REQUEST['is_luck'] == 1)
            {
                include_once(ROOT_PATH . 'wxpay/wx_hongbao.php');
                $total_amount = (100) * floatval($fmt_amount);
                $min_value    = $total_amount;
                $max_value    = $total_amount;
                $re_openid    = $db->getOne("SELECT `openid` FROM ".$hhs->table('users')." WHERE `user_id` = '".$account['user_id']."' ");

                $hongbao = new hongbao();

                $res = $hongbao->send($re_openid,$total_amount,$min_value,$max_value);
				//echo $hongbao->send($re_openid,$total_amount,$min_value,$max_value);exit;
                if(! $res){
                    $link[] = array('text' => $_LANG['go_back'], 'href'=>'javascript:history.back(-1)');
                    sys_msg('微信支付失败', 0, $link);
                }
            }
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT u.user_name FROM " . $hhs->table('users') . " AS u, " .
           $hhs->table('user_account') . " AS ua " .
           " WHERE u.user_id = ua.user_id AND ua.id = '$id' ";
    $user_name = $db->getOne($sql);
    $sql = "DELETE FROM " . $hhs->table('user_account') . " WHERE id = '$id'";
...
function account_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['keywords'])
        {
            $where .= " AND u.uname LIKE '%" . mysql_like_quote($filter['keywords']) . "%'";
            $sql = "SELECT COUNT(*) FROM " .$GLOBALS['hhs']->table('user_account'). " AS ua, ".
      ...
      $sql = "SELECT COUNT(*) FROM " .$GLOBALS['hhs']->table('user_account'). " AS ua LEFT JOIN ".
                   $GLOBALS['hhs']->table('users') . " AS u ON ua.user_id = u.user_id " . $where;
        ...
        /* 查询数据 */
        $sql  = 'SELECT ua.*, u.uname as user_name FROM ' .
            $GLOBALS['hhs']->table('user_account'). ' AS ua LEFT JOIN ' .
            $GLOBALS['hhs']->table('users'). ' AS u ON ua.user_id = u.user_id'.
