 ...
 *
 * $Author: pangbin $
 * $Id: goods_export.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
elseif ($_REQUEST['act'] == 'act_export_taobao')
{
    ...
    {
        $sql = "SELECT region_id,  region_name FROM " . $hhs->table('region') . " WHERE region_id IN ('$_CFG[shop_province]',  '$_CFG[shop_city]')";
        $arr = $db->getAll($sql);
    ...
    " FROM " . $hhs->table('goods') . " AS g ". $where;
    ...
    if (EC_CHARSET != 'utf-8')
    {
        $content = hhs_iconv(EC_CHARSET, 'utf-8', $content);
    }
...
elseif($_REQUEST['act'] == 'act_export_hhshop')
{
    /* 检查权限 */
    admin_priv('goods_export');
    ...
    $sql = "SELECT g.*, b.brand_name as brandname " .
           " FROM " . $hhs->table('goods') . " AS g LEFT JOIN " . $hhs->table('brand') . " AS b " .
    ...
    $zip->add_file(hhs_iconv(EC_CHARSET, $charset, $content), 'goods_list.csv');
...
elseif ($_REQUEST['act'] == 'act_export_paipai')
{
    ...
    if (EC_CHARSET == 'utf-8')
    {
        $zip->add_file(hhs_iconv('UTF8', 'GB2312', $content), 'goods_list.csv');
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'act_export_custom')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $sql = "SELECT `attr_id`, `attr_value` FROM " . $hhs->table('goods_attr') . " WHERE `goods_id` = '" . $row['goods_id'] . "'";
        $query = $db->query($sql);
...
{
    $sql = "SELECT `attr_id`, `cat_id`, `attr_name` FROM " . $GLOBALS['hhs']->table('attribute') . " ";
    if (!empty($cat_id))
    {
...
function set_goods_field_name($array, $lang)
{
    ...
    foreach ($array as $key => $value)
    {
        ...
        {
            $tmp_fields[$key] = $GLOBALS['db']->getOne("SELECT `attr_name` FROM " . $GLOBALS['hhs']->table('attribute') . " WHERE `attr_id` = '" . intval($value) . "'");
        }
    }
    return $tmp_fields;
}
