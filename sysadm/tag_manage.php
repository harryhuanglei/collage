 ...
 * 小舍电商 鍚庡彴鏍囩?绠＄悊
 * ============================================================================
 * * 鐗堟潈鎵€鏈 2012-2014 涓婃捣鍟嗘淳缃戠粶绉戞妧鏈夐檺鍏?徃锛屽苟淇濈暀鎵€鏈夋潈鍒┿€
 * 缃戠珯鍦板潃: http://www.baidu.com锛
 * ----------------------------------------------------------------------------
 * 杩欎笉鏄?竴涓?嚜鐢辫蒋浠讹紒鎮ㄥ彧鑳藉湪涓嶇敤浜庡晢涓氱洰鐨勭殑鍓嶆彁涓嬪?绋嬪簭浠ｇ爜杩涜?淇?敼鍜
 * 浣跨敤锛涗笉鍏佽?瀵圭▼搴忎唬鐮佷互浠讳綍褰㈠紡浠讳綍鐩?殑鐨勫啀鍙戝竷銆
 * ============================================================================
 * $Author: pangbin $
 * $Id: tag_manage.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
elseif($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if($is_insert)
    {
        $sql = 'INSERT INTO ' . $hhs->table('tag') . '(tag_id, goods_id, tag_words)' .
...
elseif ($_REQUEST['act'] == 'batch_drop')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
            $sql = "DELETE FROM " .$hhs->table('tag'). " WHERE tag_id='$id'";
            $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 鑾峰彇鍒犻櫎鐨勬爣绛剧殑鍚嶇О */
    $tag_name = $db->getOne("SELECT tag_words FROM " .$hhs->table('tag'). " WHERE tag_id = '$id'");
...
function tag_is_only($name, $tag_id, $goods_id = '')
{
    ...
    {
        $db = $GLOBALS['db'];
        $sql = 'SELECT goods_id FROM ' . $GLOBALS['hhs']->table('tag') . " WHERE tag_id = '$tag_id'";
        $row = $GLOBALS['db']->getRow($sql);
        $goods_id = $row['goods_id'];
    }

    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['hhs']->table('tag') . " WHERE tag_words = '$name'" .
...
{
    $db = $GLOBALS['db'];
    $sql = 'UPDATE ' . $GLOBALS['hhs']->table('tag') . " SET tag_words = '$name'";
    if(!empty($goods_id))
    {
...
function get_tag_list()
{
    ...
    $sql = "SELECT t.tag_id, u.user_name, t.goods_id, g.goods_name, t.tag_words ".
            "FROM " .$GLOBALS['hhs']->table('tag'). " AS t ".
            "LEFT JOIN " .$GLOBALS['hhs']->table('users'). " AS u ON u.user_id=t.user_id ".
            "LEFT JOIN " .$GLOBALS['hhs']->table('goods'). " AS g ON g.goods_id=t.goods_id ".
...
{
    $sql = 'SELECT t.tag_id, t.tag_words, t.goods_id, g.goods_name FROM ' . $GLOBALS['hhs']->table('tag') . ' AS t' .
           ' LEFT JOIN ' . $GLOBALS['hhs']->table('goods') . ' AS g ON t.goods_id=g.goods_id' .
           " WHERE tag_id = '$tag_id'";
    $row = $GLOBALS['db']->getRow($sql);
