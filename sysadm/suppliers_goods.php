 ...
 * $Id: suppliers_goods.php 17217 2014-05-12 06:29:08Z pangbin $
...
include_once(ROOT_PATH . 'languages/' . $_CFG['lang'] . '/admin/goods.php');
...

{
    ...
    $action_link = ($_REQUEST['act'] == 'list') ? add_link($code) : array('href' => 'suppliers_goods.php?act=list', 'text' => $_LANG['01_goods_list']);
    ...
    $htm_file = ($_REQUEST['act'] == 'list') ?
        ...
        'suppliers_goods_list.htm' : (($_REQUEST['act'] == 'trash') ? 'goods_trash.htm' : 'group_list.htm');
...

{
    ...
    $smarty->display('suppliers_goods_info.htm');
...

{
	...
	$goods_img_w = intval($_CFG['image_width']);

	 

	$goods_img_h = intval($_CFG['image_height']);
...

    {
		...
		if($goods_img_w && $goods_img_h && $_CFG['close_img'])

		{

			

			$goods_img_arr = getimagesize($_FILES['goods_img']['tmp_name']);

			

			

			if($goods_img_arr[0] != $goods_img_w || $goods_img_arr[1] != $goods_img_h)

			{

			

			sys_msg('商品图片上传宽高不符合,请上传'.$goods_img_w.'*'.$goods_img_h, 1, array(), false);
	...
	/*团矩形图信息*/
	if($_FILES['little_img']['tmp_name'] != '' && $_FILES['little_img']['tmp_name'] != 'none')
	{
		if(intval($_CFG['tuan_image_w']) && intval($_CFG['tuan_image_h']) && $_CFG['close_img'])
		{
			$goods_img_arr = getimagesize($_FILES['little_img']['tmp_name']);
			if($goods_img_arr[0] != intval($_CFG['tuan_image_w']) || $goods_img_arr[1] != intval($_CFG['tuan_image_h']))
			{
				sys_msg('拼团矩形图片宽高不符合, 请上传'.$_CFG['tuan_image_w'].'*'.$_CFG['tuan_image_h'], array(), false);
			}
		}
        $image ->data_dir = 'business/uploads';
        $little_img= $image->upload_image($_FILES['little_img'],$suppliers_id);
	}



	$lab_qgby = isset($_POST['lab_qgby']) ? trim($_POST['lab_qgby']) : '';



    $lab_zpbz = isset($_POST['lab_zpbz']) ? trim($_POST['lab_zpbz']) : '';



    $lab_qtth = isset($_POST['lab_qtth']) ? trim($_POST['lab_qtth']) : '';



    $lab_jkbs = isset($_POST['lab_jkbs']) ? trim($_POST['lab_jkbs']) : '';



    $lab_hwzy = isset($_POST['lab_hwzy']) ? trim($_POST['lab_hwzy']) : '';
	...
	$ts_a = isset($_POST['ts_a']) ? trim($_POST['ts_a']) : '';



	$ts_b = isset($_POST['ts_b']) ? trim($_POST['ts_b']) : '';



	$ts_c = isset($_POST['ts_c']) ? trim($_POST['ts_c']) : '';
    ...
    $is_mall = isset($_POST['is_mall']) ? intval($_POST['is_mall']) : 0;



    $is_zero = isset($_POST['is_zero']) ? intval($_POST['is_zero']) : 0;



    $is_team = isset($_POST['is_team']) ? intval($_POST['is_team']) : 0;



    $is_tejia = isset($_POST['is_tejia']) ? 1 : 0;



    $is_fresh = isset($_POST['is_fresh']) ? 1 : 0;



    $allow_fenxiao = isset($_POST['allow_fenxiao']) ? intval($_POST['allow_fenxiao'])  : 0;



    $bonus_allowed =  isset($_POST['bonus_allowed']) ? $_POST['bonus_allowed'] : 0;
    ...
    $rate_1 = $allow_fenxiao && isset($_POST['rate_1']) ? floatval($_POST['rate_1']) : 0;



    $rate_2 = $allow_fenxiao && isset($_POST['rate_2']) ? floatval($_POST['rate_2']) : 0;



    $rate_3 = $allow_fenxiao && isset($_POST['rate_3']) ? floatval($_POST['rate_3']) : 0;
	...
	$is_miao = isset($_POST['is_miao']) ? intval($_POST['is_miao']) : 0;
    ...
    {   
        ...
        if(empty($_POST['team_price']) || floatval($_POST['team_price']) <= 0.00 )
        ...
        if(empty($_POST['team_num']) || intval($_POST['team_num']) <= 0 )
    ...
    if($is_zero){
        ...
        if(empty($_POST['shop_price']) || floatval($_POST['shop_price']) <= 0.00 )
        ...
        if(empty($_POST['market_price']) || floatval($_POST['market_price']) <= 0.00 )
...

    {
...

        {
            ...
            $sql = "INSERT INTO " . $hhs->table('goods') . " (is_miao,rate_1,rate_2,rate_3,subscribe,limit_buy_one,limit_buy_bumber,lab_qgby, lab_zpbz, lab_qtth, lab_jkbs, lab_hwzy,ts_a,ts_b,ts_c,allow_fenxiao,is_fresh,is_tejia,shipping_fee,is_mall,is_zero,is_team,bonus_allowed,discount_type,discount_amount,guige,district_id,city_id,is_nearby,sales_num,little_img,team_num,team_price,goods_name, goods_name_style, goods_sn, " .
                ...
                "VALUES ('$is_miao','$rate_1','$rate_2','$rate_3','$subscribe','$limit_buy_one','$limit_buy_bumber','$lab_qgby', '$lab_zpbz', '$lab_qtth', '$lab_jkbs', '$lab_hwzy', '$ts_a', '$ts_b', '$ts_c','$allow_fenxiao','$is_fresh','$is_tejia','$shipping_fee','$is_mall','$is_zero','$is_team','$bonus_allowed','$discount_type','$discount_amount','$guige','$district_id','$city_id','$is_nearby','$sales_num','$little_img','$team_num','$team_price','$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
...

    {
        ...
        $link[0] = array('href' => 'suppliers_goods.php?act=product_list&goods_id=' . $goods_id, 'text' => $_LANG['product']);
...
elseif ($_REQUEST['act'] == 'toggle_tejia')
...

{
    ...
    if ($exc->edit("is_tejia = '$is_tejia', last_update=" .gmtime(), $goods_id))
...
elseif ($_REQUEST['act'] == 'toggle_fresh')
...

{
    ...
    if ($exc->edit("is_fresh = '$is_fresh', last_update=" .gmtime(), $goods_id))
...
/*app专享*/
elseif ($_REQUEST['act'] == 'toggle_app')

{
    ...
    $is_app       = intval($_POST['val']);



    if ($exc->edit("is_app = '$is_app', last_update=" .gmtime(), $goods_id))

    {

        clear_cache_files();

        make_json_result($is_app);
...

{
...

    {
        ...
        $link[] = array('href' => 'suppliers_goods.php?act=trash', 'text' => $_LANG['11_goods_trash']);
...
elseif ($_REQUEST['act'] == 'check') {
    ...
    $check_desc = $db->getOne("select check_desc from " . $hhs->table('goods') . " where  goods_id='$id'");
    ...
    $smarty->display('goods_check.htm');
    ...
    $sql = $db->query("update " . $hhs->table('goods') . " set is_check='$is_check',check_desc='$check_desc' where goods_id='$id'");
    ...
    $rows = $db->getRow("select phone,email,suppliers_name from " . $hhs->table('suppliers') . " where suppliers_id=(select suppliers_id from " . $hhs->table('goods') . " where goods_id = " . $id . ") ");
    ...
    if ($rows) {
        ...
        if ($is_email == 'on') {
            ...
            if ($is_check == 0) {
                ...
                $content = '尊敬的商家，您的商品正在审核中，请耐心等待！' . $desc;
            ...
            if ($is_check == 1) {
                ...
                $content = '尊敬的商家，您提交的商品申请已审核通过！备注信息：' . $desc;
            ...
            if ($is_check == 2) {
                ...
                $content = '尊敬的商家，您的商品未通过审核，详情请致电客服' . $_CFG['service_phone'] . '' . $desc;
            ...
            send_mail($_CFG['shop_name'] . $rows['suppliers_name'], $rows['email'], '商品审核通知', $content, 1);
        ...
        if ($is_sms == 'on') {
            ...
            if ($is_check == 1) {
                ...
                $content = '尊敬的商家，您提交的商品申请已审核通过!' . $desc;
    ...
    $links = array(
        ...
        array(
            ...
            'href' => 'suppliers_goods.php?act=list&page=' . $page,
...

{
    ...
    $goods_list = goods_list($is_delete, ($code=='') ? 1 : 0," and suppliers_id >0 ");
    ...
    $tpl = $is_delete ? 'goods_trash.htm' : 'suppliers_goods_list.htm';
...

{
...

    {
        ...
        $url = 'suppliers_goods.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...

{
    ...
    $url = 'suppliers_goods.php?act=query&' . str_replace('act=restore_goods', '', $_SERVER['QUERY_STRING']);
...

{
    ...
    $url = 'suppliers_goods.php?act=query&' . str_replace('act=drop_goods', '', $_SERVER['QUERY_STRING']);
...

{
...

    {
        ...
        $link[] = array('href' => 'suppliers_goods.php?act=list', 'text' => $_LANG['cannot_found_goods']);
...

    {
        ...
        $link[] = array('href' => 'suppliers_goods.php?act=list', 'text' => $_LANG['01_goods_list']);
...

    {
        ...
        $link[] = array('href' => 'suppliers_goods.php?act=edit&goods_id=' . $goods_id, 'text' => $_LANG['edit_goods']);
    ...
    $smarty->assign('action_link',  array('href' => 'suppliers_goods.php?act=list', 'text' => $_LANG['01_goods_list']));
...

{
...

    {
        ...
        $url = 'suppliers_goods.php?act=product_query&' . str_replace('act=product_remove', '', $_SERVER['QUERY_STRING']);
...

{
...

    {
         ...
         $link[] = array('href' => 'suppliers_goods.php?act=add', 'text' => $_LANG['02_goods_add']);
         ...
         $link[] = array('href' => 'suppliers_goods.php?act=product_list&goods_id=' . $product['goods_id'], 'text' => $_LANG['18_product_list']);
...

    {
         ...
         $link[] = array('href' => 'suppliers_goods.php?act=list&uselastfilter=1', 'text' => $_LANG['01_goods_list']);



         $link[] = array('href' => 'suppliers_goods.php?act=edit&goods_id=' . $product['goods_id'], 'text' => $_LANG['edit_goods']);
...

{
    ...
    $link[] = array('href' => 'suppliers_goods.php?act=product_list&goods_id=' . $_POST['goods_id'], 'text' => $_LANG['item_list']);
...

{
    ...
    $href = 'suppliers_goods.php?act=list&page='.$_REQUEST['page'];
...

{
    ...
    $href = 'suppliers_goods.php?act=add';
