 ...
 * ==========================================================
 * $Author: pangbin $
 * $Id: cron.php 17217 2014-05-12 06:29:08Z pangbin $
 */
...
admin_priv('cron');
$exc = new exchange($hhs->table('crons'), $db, 'cron_code', 'cron_name');
if ($_REQUEST['act'] == 'list')
{
    $cron_list = array();
    $sql = "SELECT * FROM " . $hhs->table('crons');
    $res = $db->query($sql);
...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $hhs->table('crons') .
        ...
        $sql = "INSERT INTO " . $hhs->table('crons') . " (cron_code, cron_name, cron_desc, cron_config, nextime, day, week, hour, minute, run_once, allow_ip, alow_files)" .
...
elseif ($_REQUEST['act'] == 'edit')
{
    if (empty($_POST['step']))
    {
        ...
        $sql = "SELECT * FROM " . $hhs->table('crons') . " WHERE cron_code = '$_REQUEST[code]'";
        $cron = $db->getRow($sql);
        if (empty($cron))
        {
            $links[] = array('text' => $_LANG['back_list'], 'href' => 'cron.php?act=list');
...
{
    $sql = "DELETE FROM " . $hhs->table('crons') .
...
elseif ($_REQUEST['act'] == 'do')
{
    ...
    {
        $cron = array();
        $sql = "SELECT cron_config FROM " . $hhs->table('crons') . " WHERE cron_code = '$_REQUEST[code]'";
        $temp = $db->getRow($sql);
