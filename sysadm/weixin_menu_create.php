...
$ret = $db->getRow("SELECT * FROM ". $GLOBALS['hhs']->table('weixin_config') ." WHERE `id` = 1");
...
function getmenu() {
 ...
 $sql="SELECT cat_id, cat_name, weixin_type, links, weixin_key as wkey FROM ". $GLOBALS['hhs']->table('weixin_menu') ." WHERE parent_id = 0 order by sort_order ASC,cat_id desc";
...
$msg = curl_menu(ACCESS_TOKEN, preg_replace("#\\\u([0-9a-f]{4})#ie", "iconv('UCS-2BE', 'UTF-8', pack('H4', '\\1'))",  $data));
