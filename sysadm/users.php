...

{
    ...
    $sql = "SELECT a.*, c.region_name AS country_name, p.region_name AS province_name, ct.region_name AS city_name, d.region_name AS district_name ".
...

{
...

    {
        ...
        $filter['pid']   = empty($_REQUEST['pid'])    ? '' : intval($_REQUEST['pid']);

        $filter['level'] = empty($_REQUEST['level'])    ? '' : intval($_REQUEST['level']);
        ...
        if ($filter['pid']!='' && $filter['level']!='')

        {

             $ex_where .=" AND uid_".$filter['level']." = '$filter[pid]' ";
...

    {
        ...
        $user_list[$i]['level_1_num'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('users') . " WHERE uid_1 = " . $user_list[$i]['user_id']);

        $user_list[$i]['level_2_num'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('users') . " WHERE uid_2 = " . $user_list[$i]['user_id']);

        $user_list[$i]['level_3_num'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('users') . " WHERE uid_3 = " . $user_list[$i]['user_id']);
