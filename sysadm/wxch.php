...
$weixinlangtable = $GLOBALS['hhs']->table('weixin_lang');



$weixincfgtable = $GLOBALS['hhs']->table('weixin_cfg');



$weixinbonustable = $GLOBALS['hhs']->table('weixin_bonus');



$weixinpointtable = $GLOBALS['hhs']->table('weixin_point');



$weixinconfigtable = $GLOBALS['hhs']->table('weixin_config');



$weixinkeywordstable = $GLOBALS['hhs']->table('weixin_keywords');
...
if ($_REQUEST['act'] == 'wxconfig') {
    ...
    if (!empty($_POST['token'])) {
        ...
        $ret       = $db->query("UPDATE ".$weixinconfigtable." SET `token`='$token',`appid`='$appid',`appsecret`='$appsecret',`nick_name`='$nick_name',`send_name`='$send_name',`wishing`='$wishing',`act_name`='$act_name',`remark`='$remark',`client_ip`='$client_ip' WHERE `id`=1;");
        ...
        $weixin_config = $db->getRow("SELECT * FROM ".$weixinconfigtable." WHERE `id` = 1");



        $smarty->assign('config', $weixin_config);
	...
	if($_POST) {
		...
		$thistable = $hhs->prefix.'bonus_type';
