...
elseif($_REQUEST['act'] == 'install' || $_REQUEST['act'] == 'view')
{
	...
	if($view)
	{
		if(file_exists($filepath .'config/'.$type.'_config.php'))
		{
			...
			$sql = 'SELECT rank_id,rank_name FROM '.$hhs->table('user_rank').' WHERE rank_id=\''.RANK_ID.'\'';
...
elseif($_REQUEST['act']  == 'query_install' || $_REQUEST['act'] == 'update_website')
{
	...
	if($query || !$rank_id)
	{
		$sql = 'INSERT INTO '.$hhs->table('user_rank').'(`rank_name` , `discount` , `special_rank`,`show_price`) VALUES'.
				"('$rank_name' , '100' , '1','0')";
		$db->query($sql);
	...
	else
	{
		...
		{
			$sql = 'UPDATE '.$hhs->table('user_rank').' SET `rank_name` = '."'$rank_name' WHERE `rank_id`='$rank_id'";
			$db->query($sql);
		}
	}
...
elseif($_REQUEST['act'] == 'uninstall')
{
	...
	if(file_exists($filepath .'config/'.$type.'_config.php'))
	{
		include_once($filepath .'config/'.$type.'_config.php');
		if(!defined(RANK_ID))
			$db->query('DELETE FROM '.$hhs->table('user_rank').' WHERE `rank_id`=\''.RANK_ID.'\'');
...
{
	$fields = $db->getCol('DESC '.$hhs->table('users'));
	$init = true;
	...
	if($init)
	{
		$sql = 'ALTER TABLE '.$hhs->table('users').' ADD `aite_id` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER `user_id`';
		$db->query($sql);
	}
	assign_query_info();
