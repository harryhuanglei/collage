...
/*初始化数据交换对象 */
$exc   = new exchange($hhs->table("exchange_goods"), $db, 'goods_id', 'exchange_integral');
//$image = new cls_image();
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$hhs->table('exchange_goods')."(goods_id, exchange_integral, is_exchange, is_hot) ".
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT eg.goods_id, eg.exchange_integral,eg.is_exchange, eg.is_hot, g.goods_name ".
           " FROM " . $hhs->table('exchange_goods') . " AS eg ".
...
{
    check_authz_json('exchange_remove');
