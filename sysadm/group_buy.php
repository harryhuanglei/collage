 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: group_buy.php 17217 2014-05-12 06:29:08Z pangbin $
 */
...
elseif ($_REQUEST['act'] =='insert_update')
{
    ...
    if (isset($_POST['finish']))
    {
        ...
        /* 结束团购活动，修改结束时间为当前时间 */
        $sql = "UPDATE " . $hhs->table('goods_activity') .
                " SET end_time = '" . gmtime() . "' " .
    ...
    elseif (isset($_POST['mail']))
    {
        ...
        while ($order = $db->fetchRow($res))
        {
            ...
            $smarty->assign('shop_url',     $hhs->url() . 'user.php?act=order_detail&order_id='.$order['order_id']);
    ...
    else
    {
        ...
        $goods_name = $db->getOne("SELECT goods_name FROM " . $hhs->table('goods') . " WHERE goods_id = '$goods_id'");
        ...
        {
            /* update */
            $db->autoExecute($hhs->table('goods_activity'), $group_buy, 'UPDATE', "act_id = '$group_buy_id'");
        ...
        {
            /* insert */
            $db->autoExecute($hhs->table('goods_activity'), $group_buy, 'INSERT');
...
elseif ($_REQUEST['act'] == 'batch_drop')
{
    if (isset($_POST['checkboxes']))
    {
        ...
        foreach ($_POST['checkboxes'] AS $key => $id)
        {
            ...
            {
                /* 删除团购活动 */
                $sql = "DELETE FROM " . $GLOBALS['hhs']->table('goods_activity') .
...
elseif ($_REQUEST['act'] == 'edit_deposit')
{
    ...
    $sql = "SELECT ext_info FROM " . $hhs->table('goods_activity') .
...
function group_buy_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('goods_activity') .
...
{
    $sql = "SELECT * FROM " . $GLOBALS['hhs']->table('goods_activity') .
