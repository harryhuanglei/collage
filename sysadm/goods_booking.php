 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: goods_booking.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $db->query("DELETE FROM " .$hhs->table('booking_goods'). " WHERE rec_id='$id'");
...
if ($_REQUEST['act'] =='update')
{
    ...
    $sql = "UPDATE  ".$hhs->table('booking_goods').
    ...
    if (!empty($_POST['send_email_notice']) or isset($_POST['remail']))
    {
        ...
        $sql = 'SELECT bg.email, bg.link_man, bg.goods_id, g.goods_name ' .
               'FROM ' .$hhs->table('booking_goods'). ' AS bg, ' .$hhs->table('goods'). ' AS g ' .
...
function get_bookinglist()
{
    ...
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['hhs']->table('booking_goods'). ' AS bg, '.
    ...
    $sql = 'SELECT bg.rec_id, bg.link_man, g.goods_id, g.goods_name, bg.goods_number, bg.booking_time, bg.is_dispose '.
            'FROM ' .$GLOBALS['hhs']->table('booking_goods'). ' AS bg, ' .$GLOBALS['hhs']->table('goods'). ' AS g '.
            "WHERE bg.goods_id = g.goods_id $where " .
...
function get_booking_info($id)
{
    ...
    $sql ="SELECT bg.rec_id, bg.user_id, IFNULL(u.user_name, '$_LANG[guest_user]') AS user_name, ".
            ...
            "LEFT JOIN " . $hhs->table('goods') . " AS g ON g.goods_id=bg.goods_id ".
            "LEFT JOIN " . $hhs->table('users') . " AS u ON u.user_id=bg.user_id ".
            "WHERE bg.rec_id ='$id'";
