...
//        break;
    ...
    case 'send_sms' :
        ...
        if ($send_rank != 0)
        {
            ...
            if($rank_array['0'] == 1)
            {
                $sql = 'SELECT mobile_phone FROM ' . $hhs->table('users') . "WHERE mobile_phone <>'' ";
                $row = $db->query($sql);
            ...
            {
                $rank_sql = "SELECT * FROM " . $hhs->table('user_rank') . " WHERE rank_id = '" . $rank_array['1'] . "'";
                $rank_row = $db->getRow($rank_sql);
                //$sql = 'SELECT mobile_phone FROM ' . $hhs->table('users') . "WHERE mobile_phone <>'' AND rank_points > " .$rank_row['min_points']." AND rank_points < ".$rank_row['max_points']." ";

                if($rank_row['special_rank']==1) 
                {
                    $sql = 'SELECT mobile_phone FROM ' . $hhs->table('users') . " WHERE mobile_phone <>'' AND user_rank = '" . $rank_array['1'] . "'";
                }
                else
                {
                    $sql = 'SELECT mobile_phone FROM ' . $hhs->table('users') . "WHERE mobile_phone <>'' AND rank_points > " .$rank_row['min_points']." AND rank_points < ".$rank_row['max_points']." ";
        ...
        $temp_phone=explode(',',$phone);

        if(count($temp_phone)>199)
        {
            $phone_key=0;
            $i=0;
            foreach($temp_phone as $val)
            {
                if($i<199)
                {
                   $i++;
                ...
                $t_phone[$phone_key]['phone'].=$val.',';
            }
            foreach ($t_phone as $val_p)
            {
                //var_dump($val_p);
                $result = $sms->send($val_p['phone'], $msg, $send_date, $send_num = 13);
            }
        }
        else
        {
