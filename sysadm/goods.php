...

{
    ...
    $bonus_list = $db->getAll("SELECT `type_id`,`type_name` from ".$hhs->table('bonus_type')." WHERE suppliers_id = 0 AND free_all=1");
...

    {
...

        {
            ...
            $sql = "SELECT 0 AS goods_id, attr_id, attr_value, attr_price,attr_team_price " .
...

{
    ...
    if(intval($_POST['team_price'])<1){

        $_POST['is_luck'] = 0;

    }

    $is_luck        = isset($_POST['is_luck']) ? intval($_POST['is_luck']) : 0;

    if($is_luck >0 ){

        $luck_times     = intval($_POST['luck_times'])>0?intval($_POST['luck_times']):'';

        $max_luck_times = intval($_POST['max_luck_times'])>0?intval($_POST['max_luck_times']):'';



        $_POST['team_price']   = $_POST['luck_price'];

        // $_POST['goods_number'] = $_REQUEST['act'] == 'update' ? $_POST['goods_number'] : $_POST['luck_number'];//

        $_POST['team_num']     = $_POST['luck_number'];//
    ...
    $allow_sharej = isset($_POST['allow_sharej']) ? intval($_POST['allow_sharej'])  : 0;

    $share_j = $allow_sharej && isset($_POST['share_j']) ? intval($_POST['share_j']) : 0;
    ...
    {
		/*获取图片信息*/
		if($goods_img_w > 0 && $goods_img_h > 0 && $_CFG['close_img'])
		{
    ...
    $is_promote = $is_miao ? 1: 0;//empty($promote_price) ? 0 : 1;
    ...
    $spe_region           = @implode(',', $_REQUEST['spe_region']);

    $default_shipping_id  = empty($_POST['default_shipping_id']) ? '0' : intval($_POST['default_shipping_id']);

    $default_shipping_fee = empty($_POST['default_shipping_fee']) ? '0.00' : floatval($_POST['default_shipping_fee']);

    $default_step_fee     = empty($_POST['default_step_fee']) ? '0.00' : floatval($_POST['default_step_fee']);

    $spe_shipping_fee     = empty($_POST['spe_shipping_fee']) ? '0.00' : floatval($_POST['spe_shipping_fee']);

    $spe_step_fee         = empty($_POST['spe_step_fee']) ? '0.00' : floatval($_POST['spe_step_fee']);
    ...
    $bonus_free_all = empty($_POST['bonus_free_all']) ? '0' : intval($_POST['bonus_free_all']);
    /* 入库 */
    if ($is_insert)

    {
...

        {
            ...
            $sql = "INSERT INTO " . $hhs->table('goods') . " (allow_sharej,share_j,rate_1,rate_2,rate_3,bonus_free_all,luck_times,max_luck_times,dbgz,is_luck,lab_qgby, lab_zpbz, lab_qtth, lab_jkbs, lab_hwzy,ts_a,ts_b,ts_c,is_miao,express,spe_region,default_shipping_id,default_shipping_fee,default_step_fee,spe_shipping_fee,spe_step_fee,allow_fenxiao,is_fresh,is_tejia,shipping_fee,is_mall,is_zero,is_team,bonus_allowed,discount_type,discount_amount,subscribe,district_id,city_id,limit_buy_one,limit_buy_bumber,is_check,is_nearby,sales_num,little_img,team_num,team_price,goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id,guige)" .

                "VALUES ('$allow_sharej','$share_j','$rate_1','$rate_2','$rate_3','$bonus_free_all',1,'$max_luck_times','$dbgz','$is_luck','$lab_qgby', '$lab_zpbz', '$lab_qtth', '$lab_jkbs', '$lab_hwzy', '$ts_a', '$ts_b', '$ts_c','$is_miao','$express','$spe_region','$default_shipping_id','$default_shipping_fee','$default_step_fee','$spe_shipping_fee','$spe_step_fee','$allow_fenxiao','$is_fresh','$is_tejia','$shipping_fee','$is_mall','$is_zero','$is_team','$bonus_allowed','$discount_type','$discount_amount','$subscribe','$district_id','$city_id','$limit_buy_one','$limit_buy_bumber',1,'$is_nearby' ,'$sales_num','$little_img','$team_num','$team_price','$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id','$guige')";
...

        {
            ...
            $sql = "INSERT INTO " . $hhs->table('goods') . " (allow_sharej,share_j,rate_1,rate_2,rate_3,bonus_free_all,luck_times,max_luck_times,dbgz,is_luck,lab_qgby, lab_zpbz, lab_qtth, lab_jkbs, lab_hwzy,ts_a,ts_b,ts_c,is_miao,express,spe_region,default_shipping_id,default_shipping_fee,default_step_fee,spe_shipping_fee,spe_step_fee,allow_fenxiao,is_fresh,is_tejia,shipping_fee,is_mall,is_zero,is_team,bonus_allowed,discount_type,discount_amount,subscribe,district_id,city_id.limit_buy_one,limit_buy_bumber,suppliers_id,is_check,is_nearby,sales_num,little_img,team_num,team_price,goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,guige)" .

                "VALUES ('$allow_sharej','$share_j','$rate_1','$rate_2','$rate_3','$bonus_free_all',1,'$max_luck_times','$dbgz','$is_luck','$lab_qgby', '$lab_zpbz', '$lab_qtth', '$lab_jkbs', '$lab_hwzy', '$ts_a', '$ts_b', '$ts_c','$is_miao','$express','$spe_region','$default_shipping_id','$default_shipping_fee','$default_step_fee','$spe_shipping_fee','$spe_step_fee','$allow_fenxiao','$is_fresh','$is_tejia','$shipping_fee','$is_mall','$is_zero','$is_team','$bonus_allowed','$discount_type','$discount_amount','$subscribe','$district_id','$city_id','$limit_buy_one','$limit_buy_bumber','$suppliers_id',1,'$is_nearby' ,'$sales_num','$little_img','$team_num','$team_price','$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$guige')";
...

    {
        ...
        $sql = "UPDATE " . $hhs->table('goods') . " SET " .
                ...
                "max_luck_times = '$max_luck_times', " . 

		"dbgz = '$dbgz', " . 



                "bonus_free_all  = '$bonus_free_all', " . 

                "guige = '$guige', " . 

                "spe_region           = '$spe_region'," .

                "default_shipping_id  = '$default_shipping_id'," .

                "default_shipping_fee = '$default_shipping_fee'," .

                "default_step_fee     = '$default_step_fee'," .

                "spe_shipping_fee     = '$spe_shipping_fee'," .

                "spe_step_fee         = '$spe_step_fee'," .
        ...
        if(isset($_POST['attr_id_list']))
        {
            foreach ($_POST['attr_id_list'] AS $key => $attr_id)
            {
                ...
                if($_FILES['attr_img_list']['error'][$key] == 0)
                {
                    ...
                    $goods_attr_img = $image->upload_image($files_attr_p);
                }
                /*属性照片上传end*/
			//上传文件类型列表
                           /*$uptypes=array(
                       ...
                       }else
                       {
                          ...
                          }else
                          {
                            ...
                            if (!is_uploaded_file($filename))
                            {
                             ...
                             if(!in_array($_FILES["attr_img_list"]["type"][$key], $uptypes))  //检查文件类型
                            {
                               echo "";exit;
                            }
    ...
    handle_express($goods_id,$express);
...

{
    ...
    $href = 'goods.php?act=list&page='.$_REQUEST['page'];
