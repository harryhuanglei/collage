 ...
 * ==========================================================
 * $Author: pangbin $
 * $Id: search_log.php 17217 2014-05-12 06:29:08Z pangbin $
 */
...
function get_search_log()
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('keywords') . " WHERE  searchengine='hhshop' $where";
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    $logdb = array();
    $filter = page_and_size($filter);
    $sql = "SELECT * FROM " . $GLOBALS['hhs']->table('keywords') .
           " WHERE  searchengine='hhshop' $where" .
