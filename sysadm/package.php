...
if ($_REQUEST['act'] == 'add')
{
    ...
    $sql = "DELETE FROM " .$hhs->table('package_goods') .
...
elseif ($_REQUEST['act'] == 'add_package_goods')
{
    ...
    foreach ($fittings AS $val)
    {
        ...
        $sql = "INSERT INTO " . $hhs->table('package_goods') . " (package_id, goods_id, product_id, goods_number, admin_id) " .
...
{
    $sql = "UPDATE " . $GLOBALS['hhs']->table('package_goods') . " SET " .
