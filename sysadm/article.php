 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: article.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
/*初始化数据交换对象 */
$exc   = new exchange($hhs->table("article"), $db, 'article_id', 'title');
//$image = new cls_image();
...
if ($_REQUEST['act'] == 'add')
{
    ...
    /* 清理关联商品 */
    $sql = "DELETE FROM " . $hhs->table('goods_article') . " WHERE article_id = 0";
    $db->query($sql);
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$hhs->table('article')."(title, cat_id, article_type, is_open, author, ".
                "author_email, keywords, content, add_time, file_url, open_type, link, description,wx_url) ".
            "VALUES ('$_POST[title]', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]', ".
                ...
                "'$add_time', '$file_url', '$open_type', '$_POST[link_url]', '$_POST[description]', '$_POST[wx_url]')";
    $db->query($sql);

    /* 处理关联商品 */
    $article_id = $db->insert_id();
    $sql = "UPDATE " . $hhs->table('goods_article') . " SET article_id = '$article_id' WHERE article_id = 0";
    $db->query($sql);
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    /* 取文章数据 */
    $sql = "SELECT * FROM " .$hhs->table('article'). " WHERE article_id='$_REQUEST[id]'";
...
if ($_REQUEST['act'] =='update')
{
    ...
    $sql = "SELECT file_url FROM " . $hhs->table('article') . " WHERE article_id = '$_POST[id]'";
    ...
    if ($exc->edit("title='$_POST[title]', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]', is_open='$_POST[is_open]', author='$_POST[author]', author_email='$_POST[author_email]', keywords ='$_POST[keywords]', file_url ='$file_url', open_type='$open_type', content='$_POST[FCKeditor1]', link='$_POST[link_url]', description = '$_POST[description]', wx_url = '$_POST[wx_url]'", $_POST['id']))
    {
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除原来的文件 */
    $sql = "SELECT file_url FROM " . $hhs->table('article') . " WHERE article_id = '$id'";
    $old_url = $db->getOne($sql);
    ...
    if ($exc->drop($id))
    {
        $db->query("DELETE FROM " . $hhs->table('comment') . " WHERE " . "comment_type = 1 AND id_value = $id");
...
elseif ($_REQUEST['act'] == 'add_link_goods')
{
    ...
    if ($article_id == 0)
    {
        $article_id = $db->getOne('SELECT MAX(article_id)+1 AS article_id FROM ' .$hhs->table('article'));
    }

    foreach ($add_ids AS $key => $val)
    {
        $sql = 'INSERT INTO ' . $hhs->table('goods_article') . ' (goods_id, article_id) '.
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        if ($_POST['type'] == 'button_remove')
        {
            ...
            /* 删除原来的文件 */
            $sql = "SELECT file_url FROM " . $hhs->table('article') .
...
elseif ($_REQUEST['act'] == 'push')
{
    ...
    /* 取app用户数据 */
    $user_list = get_push_user();

    $smarty->assign('user_list', $user_list['user_list']);
    $smarty->assign('article',     $article);
    $smarty->assign('form_action', 'push_act');
    assign_query_info();
    $smarty->display('article_push.htm');
}
/*------------------------------------------------------ */
//-- APP推送行为 【单个或多个】
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'push_act')
{
    ...
    $sql = "SELECT * FROM " .$hhs->table('article'). " WHERE article_id='$_REQUEST[id]'";
    $article = $db->GetRow($sql);
    $tokens = trim($_REQUEST['tokens']);

    /* 调用友盟push类推送到友盟 */
    include_once(ROOT_PATH . 'umeng/umeng.php');

    if(strlen($tokens) == 44){
        /*安卓*/
        $umeng = new Umeng($_CFG['umeng_android_key'],$_CFG['umeng_android_secret']);
        $result =  $umeng ->sendAndroidUnicast($tokens,'小舍微营销',$article['title'],$article['title'],'article','','go_app');
    }elseif(strlen($tokens) == 64){
        /*苹果*/
        $umeng = new Umeng($_CFG['umeng_ios_key'],$_CFG['umeng_ios_secret']);
        $result =  $umeng ->sendIOSUnicast($tokens,$article['title'],'article');
    }
    exit;
...
{
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('goods_article') .
...
function get_article_goods($article_id)
{
    ...
    $sql  = 'SELECT g.goods_id, g.goods_name'.
            ' FROM ' . $GLOBALS['hhs']->table('goods_article') . ' AS ga'.
...
function get_articleslist()
{
    ...
    if ($result === false)
    {
        ...
        /* 文章总数 */
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['hhs']->table('article'). ' AS a '.
               'LEFT JOIN ' .$GLOBALS['hhs']->table('article_cat'). ' AS ac ON ac.cat_id = a.cat_id '.
               'WHERE 1 ' .$where;
...
function get_push_user(){
    ...
    if ($result === false)
    {
        ...
        $ex_where = " WHERE devicetoken <> '' ";
        $filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('users') . $ex_where);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT user_id,user_name,reg_time,devicetoken " . " FROM " . $GLOBALS['hhs']->table('users') . $ex_where . " ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] . " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
