 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: admin_logs.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 查询IP地址列表 */
    $ip_list = array();
    $res = $db->query("SELECT DISTINCT ip_address FROM " .$hhs->table('admin_log'));
...
if ($_REQUEST['act'] == 'batch_drop')
{
    ...
    if ($drop_type_date)
    {
        ...
        elseif ($_POST['log_date'] > '0')
        {
            ...
            $sql = "DELETE FROM " .$hhs->table('admin_log').$where;
            $res = $db->query($sql);
            if ($res)
            {
    ...
    else
    {
        ...
        {
            $sql = "DELETE FROM " .$hhs->table('admin_log'). " WHERE log_id = '$id'";
            $result = $db->query($sql);
...
function get_admin_logs()
{
    ...
    /* 获得总记录数据 */
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['hhs']->table('admin_log'). ' AS al ' . $where;
    ...
    /* 获取管理员日志记录 */
    $list = array();
    $sql  = 'SELECT al.*, u.user_name FROM ' .$GLOBALS['hhs']->table('admin_log'). ' AS al '.
            'LEFT JOIN ' .$GLOBALS['hhs']->table('admin_user'). ' AS u ON u.user_id = al.user_id '.
