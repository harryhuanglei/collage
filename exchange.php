 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: exchange.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('exchange_goods.dwt', $cache_id))
    {
        ...
        else
        {
            ...
            /* 上一个商品下一个商品 */
            $sql = "SELECT eg.goods_id FROM " .$hhs->table('exchange_goods'). " AS eg," . $GLOBALS['hhs']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND eg.goods_id > " . $goods['goods_id'] . " AND eg.is_exchange = 1 AND g.is_delete = 0 LIMIT 1";
            ...
            $sql = "SELECT max(eg.goods_id) FROM " . $hhs->table('exchange_goods') . " AS eg," . $GLOBALS['hhs']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND eg.goods_id < ".$goods['goods_id'] . " AND eg.is_exchange = 1 AND g.is_delete = 0";
...
elseif ($_REQUEST['act'] == 'buy')
{
    ...
    if (!empty($sphhs))
    {
        $_sphhs = explode(',', $sphhs);

        $product_info = get_products_info($goods_id, $_sphhs);
    }
    ...
    if((!empty($sphhs)) && ($product_info['product_number'] == 0) && ($_CFG['use_storage'] == 1))
    {
        show_message($_LANG['eg_error_number'], array($_LANG['back_up_page']), array($back_act), 'error');
    }
    ...
    $sql = "SELECT a.attr_name, g.attr_value " .
            ...
            "AND g.goods_attr_id " . db_create_in($sphhs);
    $res = $db->query($sql);
    ...
    $db->autoExecute($hhs->table('cart'), $cart, 'INSERT');

    /* 记录购物流程类型：团购 */
    $_SESSION['exchange_integral'] = $goods['exchange_integral'];
    $_SESSION['flow_type'] = CART_EXCHANGE_GOODS;
    $_SESSION['extension_code'] = 'exchange_goods';
    $_SESSION['extension_id'] = $goods_id;

    /* 进入收货人页面 */
    hhs_header("Location: ./flow.php?step=checkout&as=change\n");
    exit;
}
...
{
    return $GLOBALS['db']->getRow('SELECT keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['hhs']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.shop_price, eg.exchange_integral, ' .
                'g.goods_type, g.goods_brief, g.goods_thumb, g.goods_number , g.goods_img, eg.is_hot ' .
...
function get_exchange_goods_count($children, $min = 0, $max = 0, $ext='')
{
    ...
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['hhs']->table('exchange_goods') . ' AS eg, ' .
           $GLOBALS['hhs']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND $where $ext";
...
function get_exchange_recommend_goods($type = '', $cats = '', $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
            ...
            'FROM ' . $GLOBALS['hhs']->table('exchange_goods') . ' AS eg ' .
            'LEFT JOIN ' . $GLOBALS['hhs']->table('goods') . ' AS g ON g.goods_id = eg.goods_id ' .
