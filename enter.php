...
if($_REQUEST['act'] =='check_user_name')
{
 	...
 	if($_REQUEST['act'] =='enter_act')
	{
	   ...
	   if($_FILES['certificate']['tmp_name'] != '')
	   {
		...
		chmod($dir,0777);
		$content ="欢迎您入驻商城，请等待工作人员审核，审核后您可以<a href='".$hhs->url()."business/index.php'>登录店铺管理中心</a>用户名：$data[user_name],密码：$_REQUEST[password]，进行管理您的店铺,谢谢合作！";
		send_mail($_CFG['shop_name'] . $data['suppliers_name'], $data['email'], '欢迎您入驻', $content, 1);
