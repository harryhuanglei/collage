...
$order = '';
/* 缓存编号 */
$cache_id = 'category' . '-' . $cat_id . '-' . $sort . '-' . $page;
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    assign_pager('category',$cat_id, $count, $size, $sort, $order, $page); // 分页
...
function get_mall_goods_count($cat_id){
    $where   = "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_mall = 1 ";
    $where  .= " and $cat_id";
    //获得区域级别
