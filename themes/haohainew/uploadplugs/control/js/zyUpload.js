	...
	$.fn.zyUpload = function(options,param){
		...
		if (typeof options == 'string') {
			...
			if($.isFunction(fn)){
				...
				throw ("HHS - No such method: " + options);
			}
		}
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '<form id="uploadForm" action="'+para.url+'" method="post" enctype="multipart/form-data">';
					html += '	<div class="upload_box">';
					html += '		<div class="">'; //移除样式 upload_main single_main
				          html += '			<div class="status_bar" style="display:none;" >';
				   //       html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
				          html += '				<div class="btns">';
				  ...
				  //        html += '					<div class="upload_btn">开始上传</div>';
				          html += '				</div>';
				          html += '			</div>';
				          html += '			<div id="preview" class=" ">'; //移除样式upload_preview
					html += '				<div class="add_upload" id="add">';
					...
					html += '						<div class="uploadImg" id="up" style="width:'+imgWidth+'px">';
					html += '							<img class="upload_image"  src="themes/haohainew/uploadplugs/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width);" />';
					html += '						</div>';
					html += '					</a>';
					html += '				</div>';
					html += '			</div>';
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 5242880) {
						alert('您的"'+ file.name +'"文件过大');	
					} else {
						// 在这里需要判断当前所有文件中
						arrFiles.push(file);	
					}
				}
				var filesnum = arrFiles.length;
				if(filesnum>3){
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "themes/haohainew/uploadplugs/control/images/fileType/";
				...
				//	html += '			<p class="file_name">' + file.name + '</p>';
					...
					html += '		<div class="uploadImg" id="up" style="width:'+imgWidth+'px;">';				
					html += '			<img id="uploadImage_'+file.index+'" class="upload_image" src="' + e.target.result + '" style="height:50px; width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width);" />';                                                                 
					html += '		</div>';
					html += '	</a>';
					...
					html += '	</div>';
					html += '	<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" href="#" class="imgBox">';
					html += '		<div class="uploadImg"  style="width:'+imgWidth+'px" style="height:'+imgHeight+'px">';				
