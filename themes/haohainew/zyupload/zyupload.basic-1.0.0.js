...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="themes/haohainew/zyupload/skins/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>'; 
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
...
//								}
			...
			this.addEvent = function(){
				...
				// 绑定上传点击事件
				//$(".upload_btn").bind("click", function(e){//by houwei 2016-12-16
				$(".fabu_add_btn").bind("click", function(e){
					var  cont = $('#fabu_cont').val();
					if(cont.length > 0){
						// 判断当前是否有文件需要上传
