...
/*------------------------------------------------------ */
    ...
    {
        $WHERE = " g.goods_id not in (".$luckdraw_goods_id_list.") and ";
    }
    $sql   = "SELECT COUNT(*) FROM " .$hhs->table('goods'). " AS g ".
        "WHERE ".$WHERE." g.is_luck = 0 AND g.is_miao = 0 AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in ".
    ...
    /* 查询商品 */
    $sql = "SELECT g.is_team,g.is_mall, g.goods_id,g.is_zero, g.suppliers_id, g.goods_name, g.goods_number, g.market_price, g.is_new, g.is_best, g.is_hot, g.team_num,g.team_price, g.shop_price AS org_price, ".
    ...
    $pager = get_pager('pcsearch.php', $pager['search'], $count, $page, $size);
