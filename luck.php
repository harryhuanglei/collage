...
if($_REQUEST['act'] == 'luck_start'){
	...
	if($luck_status == 1){
		...
		$prize_order_str = implode(',', $order_id);
			...
			foreach ($complete_team_sign_user as $k=> $v){
				foreach ($v as $key => $val){
					unset($key);
					$complete_team_sign_user_arr[] = $val;
				}
			}
			//获取设置的中奖数量
			...
			if($stock_num == 1){
					...
					foreach ($get_luckdraw_user_order as $k=> $v){
						foreach ($v as $key => $val){
							unset($key);
							$luckdraw_user_order_arr[] = $val;
						}
					}
					d($luckdraw_user_order_arr);
					//抽取中奖订单
	...
	foreach ($prize_order_list['prize_order_list'] as $key => $val){
		$prize_order_list['prize_order_list'][$key]['add_time'] = local_date('Y-m-d H:i:s',$val['add_time']);
	}
	//查看是否已经发奖
	...
	$smarty->assign('prize_order_list',$prize_order_list['prize_order_list']);
...
if($_REQUEST['act'] == 'give_prize'){
	...
	$prize_order_list = get_prize_order_list($prize_order_str);
	//给中奖的用户发中奖通知
	foreach ($prize_order_list['prize_order_list'] as $k => $v){
...
elseif ($_REQUEST['act'] == 'download')
{
	...
	if(!empty($order_list['prize_order_list'])){

		foreach($order_list['prize_order_list'] as $k=>$v){
...
function get_prize_order_list($prize_order_str){
	...
	$prize_order_list_sql = "select count(*) from ".$GLOBALS['hhs']->table('goods')." as g INNER JOIN ".$GLOBALS['hhs']->table('order_info')." as o where o.extension_id = g.goods_id AND o.order_id in (".$prize_order_str.")";
	$filter['record_count'] = $GLOBALS['db']->getOne($prize_order_list_sql);
