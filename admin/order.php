...
elseif ($_REQUEST['act'] == 'list' || 'refund_list' == $_REQUEST['act'])
{
    ...
    foreach($order_list['orders'] as $value){
        if($value['shipping_code']!='cac'){
            $order_list1[] = $value;
        }elseif($value['shipping_code']=='cac'){
            $order_list2[] = $value;
        }
    }
   
    $smarty->assign('order_list1',   $order_list1);
    $smarty->assign('order_list2',   $order_list2);
...
//商家订单
elseif ($_REQUEST['act'] == 'suppliers_list' )
{
    /* 检查权限 */
    admin_priv('order_view');
    $refund_ex = 'refund_list' == $_REQUEST['act'] ? " refund_status>'0'" : "";
            ...
            if(!empty($list)){
                foreach($list as $t) {
                    ...
                    if(!empty($province)&&!empty($city)&&!empty($district)){
                        ...
                        address='$t[14]',mobile='$t[15]',consignee='$t[16]',
                        shipping_id='$t[18]',shipping_name='$t[19]',invoice_no='$t[20]' ";
...
elseif ($_REQUEST['act'] == 'to_shippingtest')
{
       ...
       // print_r($goods_list);
        ...
        if ($goods_list)
        {
			...
			foreach ($goods_list as $key=>$goods_value)
            {
                ...
                {
                    $goods_list[$key]['sended'] = $goods_value['send_number'];// 
                    $goods_list[$key]['sended'] = $goods_value['goods_number']; //商品购买量 //?????????
                    $goods_list[$key]['send'] = $goods_value['goods_number'] - $goods_value['send_number'];
					$goods_list[$key]['readonly'] = '';
                    /* 是否缺货 */
                    if ($goods_value['storage'] <= 0 && $_CFG['use_storage'] == '1'  && $_CFG['stock_dec_time'] == SDT_SHIP)
		...
		$delivery['add_time'] = trim($order['pay_time']);
		...
		$delivery['mobile'] = trim($order['mobile']); //address
   ...
   $msg = '';
	  ...
	  // print_R($goods_list);die;
        ...
        /* 获取当前操作员 */
        $delivery['action_user'] = $db->getOne("SELECT suppliers_name from " . $hhs->table('suppliers') . " where suppliers_id='{$_SESSION['suppliers_id']}'");
        ...
        if ($delivery_id)
        {
            ...
            if (!empty($goods_list))
            {
                ...
                {
                    // 商品（实货）（虚货）
                    if (empty($value['extension_code']) || $value['extension_code'] == 'team_goods')
                    {
                        $delivery_goods = array('delivery_id' => $delivery_id,
                                                'goods_id' => $value['goods_id'],
                                                'product_id' => $value['product_id'],
        ...
        if (true)
        {
            ...
            /* 更新发货时间 */
            $order_finish = get_order_finish($order_id);
            $shipping_status = SS_SHIPPED; //发货中，处理分单
            $arr['order_status'] = $order_finish ? OS_SPLITED : OS_SPLITING_PART; // 全部分单、部分分单
            $arr['shipping_status']     = $shipping_status;
            update_order($order_id, $arr);
		}
    ...
    $virtual_goods = array();
    $delivery_stock_sql = "SELECT DG.goods_id, DG.is_real, DG.product_id, SUM(DG.send_number) AS sums, IF(DG.product_id > 0, P.product_number, G.goods_number) AS storage, G.goods_name, DG.send_number
        FROM " . $hhs->table('delivery_goods') . " AS DG, " . $hhs->table('goods') . " AS G, " . $hhs->table('products') . " AS P
        WHERE DG.goods_id = G.goods_id
        AND DG.delivery_id = '$delivery_id'
        AND DG.product_id = P.product_id
        GROUP BY DG.product_id ";
    ...
    {
        $delivery_stock_sql = "SELECT DG.goods_id, DG.is_real, SUM(DG.send_number) AS sums, G.goods_number, G.goods_name, DG.send_number
        FROM " . $hhs->table('delivery_goods') . " AS DG, " . $hhs->table('goods') . " AS G
        WHERE DG.goods_id = G.goods_id
        AND DG.delivery_id = '$delivery_id'
        GROUP BY DG.goods_id ";
        $delivery_stock_result = $db->getAll($delivery_stock_sql);
    ...
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        ...
        foreach ($delivery_stock_result as $value)
        {
            ...
            if ($value['is_real'] != 0)
            {
                ...
                if (!empty($value['product_id']))
                {
                    $minus_stock_sql = "UPDATE " . $hhs->table('products') . "
                                        SET product_number = product_number - " . $value['sums'] . "
                                        WHERE product_id = " . $value['product_id'];
                    $db->query($minus_stock_sql, 'SILENT');
                }

                $minus_stock_sql = "UPDATE " . $hhs->table('goods') . "
                                    SET goods_number = goods_number - " . $value['sums'] . "
                                    WHERE goods_id = " . $value['goods_id'];
    ...
    $_delivery['invoice_no'] = $invoice_no;
    $_delivery['status'] = 0; // 0，为已发货 consignee
    $query = $db->autoExecute($hhs->table('delivery_order'), $_delivery, 'UPDATE', "delivery_id = $delivery_id", 'SILENT');
        ...
        //if($rs){
          ...
          $weixin=new class_weixin($GLOBALS['appid'],$GLOBALS['appsecret']);
            $sql = "select openid from ".$GLOBALS['hhs']->table('users')." where user_id='{$order['user_id']}'";
	    ...
	    $w_description = '订单号：'.$order['order_sn']."\r\n".'商品信息：'.$shopinfo."\r\n总金额：".$order['order_amount']."\r\n"."支付状态：已支付\r\n收货人：".$order['consignee']."\r\n收货地址：".$order['address'];  

            $weixin->send_wxmsg($openid, '发货提醒' , $w_url , $w_description);
...
function order_list($refund_ex="",$is_page=true,$is_suppliers=false)
{
    ...
    if ($result === false)
    {
       ...
       //and extension_code!='team_goods'
        ...
        /* 查询 */
        if($is_page){
            $sql = "SELECT o.surplus,o.bonus, o.package_one, o.point_id,o.transaction_id,o.team_status,o.team_sign,o.team_first, o.order_id, o.order_sn, o.add_time,o.pay_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.shipping_point," .
    ...
    foreach ($row AS $key => $value)
    {
		...
		$row[$key]['shipping_code'] = get_shipping_code($value['shipping_id']);
                    /* 取得区域名 */
    $sql = "SELECT concat(IFNULL(p.region_name, ''), " .
                "'  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region " .
            "FROM " . $GLOBALS['hhs']->table('order_info') . " AS o " .
    ...
    $row[$key]['region'] = $GLOBALS['db']->getOne($sql);
		...
		$row[$key]['point_name'] = get_point_name($value['point_id']);
...
function get_shipping_code($shipping_id)
{
    $sql = 'select shipping_code from ' . $GLOBALS['hhs']->table('shipping') . " where shipping_id='{$shipping_id}'";
