...
class integrate
{
    ...
    function integrate($cfg)
    {
        ...
        {
            $this->db_name = $GLOBALS['hhs']->db_name;
            $this->prefix = $GLOBALS['hhs']->prefix;
            $this->db = &$GLOBALS['db'];
        }
        else
        {
    ...
    function edit_user($cfg)
    {
        ...
        if ((!empty($cfg['email'])) && $this->field_email != 'NULL')
        {
            ...
            {
                // 新的E-mail
                $sql = "UPDATE " . $GLOBALS['hhs']->table('users') . " SET is_validated = 0 WHERE user_name = '$cfg[post_username]'";
                $this->db->query($sql);
            }
    ...
    function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_hhshop) && $this->is_hhshop))
        {
            ...
            if ($col)
            {
				...
				$sql = "DELETE FROM " . $GLOBALS['hhs']->table('share_info') . " WHERE " . db_create_in($col, 'user_id'); //删除用户
                $GLOBALS['db']->query($sql);				
    ...
    function set_cookie($username='', $remember= null )
    {
        ...
        elseif ($remember)
        {
            ...
            setcookie("HHS[username]", $username, $time, $this->cookie_path, $this->cookie_domain);
    ...
    function sync ($username, $password='', $md5password='')
    {
        ...
        if (empty($profile))
        {
            ...
            {
               $sql = "INSERT INTO " . $GLOBALS['hhs']->table('users').
