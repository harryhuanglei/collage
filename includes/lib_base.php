 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: lib_base.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
function send_mail($name, $email, $subject, $content, $type = 0, $notification=false)
{
    ...
    {
        $name      = hhs_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $name);
        $subject   = hhs_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $subject);
        $content   = hhs_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $content);
        $shop_name = hhs_iconv(EC_CHARSET, $GLOBALS['_CFG']['mail_charset'], $GLOBALS['_CFG']['shop_name']);
    }
 ...
 * @return  void
 **/
function hhs_header($string, $replace = true, $http_response_code = 0)
{
...
function hhs_iconv($source_lang, $target_lang, $source_string = '')
{
    static $chs = NULL;
...
function json_str_iconv($str)
{
    if (EC_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return addslashes(stripslashes(hhs_iconv('utf-8', EC_CHARSET, $str)));
        }
        elseif (is_array($str))
        {
...
function to_utf8_iconv($str)
{
    if (EC_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return hhs_iconv(EC_CHARSET, 'utf-8', $str);
        }
        elseif (is_array($str))
        {
            foreach ($str as $key => $value)
