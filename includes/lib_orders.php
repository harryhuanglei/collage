 ...
 * @param   array   $suppliers_id     



 * @param   array   $goods_list     商品id数组
...
function available_shipping_list($region_id_list, $suppliers_id = 0, $goods_id_list = array())
...

{
    ...
    if (! empty($goods_id_list)) {
        ...
        $goods_express = $GLOBALS['db']->getAll('select e.id, e.shipping_id, e.shipping_code, e.shipping_name from '.$GLOBALS['hhs']->table('goods_express').' as e,' . $GLOBALS['hhs']->table('shipping') . ' AS s WHERE s.shipping_id=e.shipping_id AND s.enabled =1 AND  e.goods_id  ' . db_create_in($goods_id_list) .' AND e.region_id ' . db_create_in($region_id_list) . ' group by e.shipping_code order by e.shipping_id desc');
        ...
        $has_express = $GLOBALS['db']->getRow('select e.id, e.shipping_id, e.shipping_code, e.shipping_name from '.$GLOBALS['hhs']->table('goods_express').' as e,' . $GLOBALS['hhs']->table('shipping') . ' AS s WHERE s.shipping_id=e.shipping_id AND s.enabled =1 AND  e.goods_id  ' . db_create_in($goods_id_list) .' ');
        ...
        if($has_express){
            ...
            $cac = $GLOBALS['db']->getAll('select e.id, e.shipping_id, e.shipping_code, e.shipping_name from '.$GLOBALS['hhs']->table('goods_express').' as e,' . $GLOBALS['hhs']->table('shipping') . ' AS s WHERE s.shipping_id=e.shipping_id AND s.enabled =1 AND s.shipping_code = "cac" AND e.goods_id  ' . db_create_in($goods_id_list) .' ');



            return $cac ? $cac : array();
    ...
    $andwhere = ' and a.`supp_id` = "'.$suppliers_id.'" ';
    ...
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            'WHERE r.region_id ' . db_create_in($region_id_list) . $andwhere . 
...
function available_point_list($region_id_list,$suppliers_id = 0)
...

{
    ...
    $andwhere = " and a.`suppliers_id` = '".$suppliers_id."' ";
    ...
    $sql = "SELECT a.*,rp.region_name as province,rc.region_name as city,rd.region_name as district " .
                ...
                " FROM " . $GLOBALS['hhs']->table('shipping_point'). " AS a left join " .
                    ...
                    " WHERE a.district " . db_create_in($region_id_list) . $andwhere;
...
function shipping_area_info($shipping_id, $region_id_list, $suppliers_id)
...

{
    ...
    $sql = 'SELECT s.shipping_code, s.shipping_name, ' .
            ...
            'AND r.region_id ' . db_create_in($region_id_list) . $andwhere .
...
function order_fee($orders, $goods, $consignee)
...

{
    ...
    foreach ($orders as $suppliers_id => $order) {
        ...
        $orders[$suppliers_id] = calc_suppliers_fee($order['order'], $goods[$suppliers_id]['goods_list'], $consignee, $suppliers_id);
...
function calc_suppliers_fee($order, $goods, $consignee, $suppliers_id)
...

{
...

    {
...

        {
            ...
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['hhs']->table('cart') . " WHERE  `extension_code` != 'package_buy' AND `is_shipping` = 0 AND  `suppliers_id` = '" . $suppliers_id. "'";
...
function cart_goods_suppliers($type = CART_GENERAL_GOODS)
...

{
...

    {
        ...
        $arr[$key]['market_price'] = price_format($value['market_price'], false);
...
function cart_amount($include_gift = true, $type = CART_GENERAL_GOODS, $suppliers_id = '-1')
...
function cart_weight_price($type = CART_GENERAL_GOODS, $suppliers_id = 0)
...

{
    ...
    $andwhere = ' and c.`suppliers_id` = "'.$suppliers_id.'" ';
    ...
    $sql = 'SELECT goods_id, goods_number, goods_price FROM ' . $GLOBALS['hhs']->table('cart') . " as c WHERE extension_code = 'package_buy' AND session_id = '" . SESS_ID . "'" . $andwhere;
    ...
    $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
                ...
                "WHERE c.session_id = '" . SESS_ID . "' " . $andwhere . 
...

{
    ...
    $sql = "SELECT g.rate_1,g.rate_2,g.rate_3,g.limit_buy_one,is_miao, g.limit_buy_bumber, g.is_fresh, g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
...

	{
		...
		if($goods['promote_start_date'] < $todayTime && $goods['promote_end_date'] > $todayTime)
...

    {
        ...
        $sql = 'select count(*) from ' . $GLOBALS['hhs']->table('order_info') . ' where user_id = ' . $_SESSION['user_id'] . ' ';
    ...
    $goods_price            = get_final_price($goods_id, $num, true, $spec,$is_miao);
...

        {
			...
			if ($goods['is_miao']) {
...

				{
					...
					$goods_price = get_final_price($goods_id, $num, true, $spec,1);
...
function user_bonus($user_id, $goods_amount = 0, $suppliers_id = 0)
...

{
   ...
   // $andwhere = ' AND b.`suppliers_id` = "'.$suppliers_id.'" ';
    ...
    $sql = "SELECT t.type_id, t.type_name,b.user_id,t.suppliers_id, t.type_money, t.min_goods_amount, b.bonus_id " .
            ...
            "WHERE t.type_id = b.bonus_type_id " . $andwhere .
...

	{
		...
		if($v['suppliers_id'] ==$suppliers_id)
...
function user_bonus_ty($user_id, $goods_amount = 0)
...

{
    ...
    $sql = "SELECT t.type_id, t.type_name,t.suppliers_id, t.type_money, t.min_goods_amount, b.bonus_id, t.tongyong " .
...

{
...

    {
...

        {
           ...
           // $row['goods_attr'] = $GLOBALS['db']->getCol($sql);
...
function get_total_bonus($suppliers_id)
...

{
    ...
    $sql = "SELECT SUM(c.goods_number * t.type_money)" .
            ...
            "AND c.suppliers_id = '".$suppliers_id."' " .
...

{
    ...
    $suppliers = cart_goods_suppliers();
...

    {
...

        {
...

            {
                ...
                $goods = $db->getRow("SELECT goods_id,goods_name,little_img FROM " . $hhs->table('goods') . " WHERE `bonus_free_all` = '".$bonus['type_id']."' ");
...
function compute_discount_amount($suppliers_id)
