...
function team_user_bonus($user_id, $goods_amount = 0,$supp_id)
{
    ...
    $sql = "SELECT t.free_all,t.type_id, t.type_name, t.type_money, t.min_goods_amount, t.suppliers_id, t.tongyong, b.bonus_id " .
            ...
            "AND b.user_id<>0 " .
            "AND (b.suppliers_id='$supp_id' or t.tongyong= 1 )" .
...
function team_user_bonus_ty($user_id, $goods_amount = 0)
{
    $day    = getdate();
    $today  = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
		...
		$shipping_code = 'select shipping_code from ' . $GLOBALS['hhs']->table('shipping') . " where shipping_id='{$order['shipping_id']}'";
		
		$order['shipping_code']       = $GLOBALS['db']->GetOne($shipping_code);
 ...
 * @return  array
 */
function order_fee($order, $goods, $consignee,$luckdraw_price)
{
    global $db,$hhs;
    /* 初始化订单的扩展code */
    ...
    if($luckdraw_price){
    	$total['amount_formated']  = price_format($luckdraw_price, false);
    }else{
 ...
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0,$rec_type=0,$team_sign=0,$luckdraw_id=0)
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;
    clear_cart(0);
    ...
    if ($num > 0)
    {
        ...
        else //购物车没有此物品，则插入
        {
            ...
            if($rec_type==5){
                ...
                if($luckdraw_id!=0){
                	$sql = "select luckdraw_price from ".$GLOBALS['hhs']->table('luckdraw')." where id = ".$luckdraw_id;
                	$luckdraw_price = $GLOBALS['db']->getOne($sql);
                	$parent['goods_price']=$luckdraw_price+spec_price($spec,true);
                }else{
...
function user_bonus($user_id, $goods_amount = 0,$team_first=false, $type_id = false, $free_all = false, $suppliers_id)
{
    ...
    $sql = "SELECT t.min_goods_amount,t.free_all,b.suppliers_id, t.type_id, t.type_name, t.type_money, b.bonus_id,t.use_start_date, t.use_end_date  " .
        "FROM " . $GLOBALS['hhs']->table('bonus_type') . " AS t," .
        $GLOBALS['hhs']->table('user_bonus') . " AS b " .
        "WHERE t.type_id = b.bonus_type_id " .
        "AND t.use_start_date <= '$today' " .
        ...
        if ($user_id > 0)
        {
            ...
            $sql = "SELECT ua.*,concat(IFNULL(p.region_name, ''), " .
                "' ', IFNULL(t.region_name, ''), ' ', IFNULL(d.region_name, '')) AS region ".
                    ...
                    " WHERE u.user_id='$user_id' AND ua.address_id = u.address_id AND ua.user_id=u.user_id";
...
//var_dump($bonus_list);exit();
    ...
    if ($bonus_list)
    {
        ...
        foreach ($bonus_list AS $bonus)
        {
			...
			{
                $w_title = '恭喜你获得好友券';
                $w_url = 'share_bonus.php?order_id='.$order_id ;
                $w_description='亲爱的'.$uname.'你好！恭喜您获得了'.$bonus['share_number'].'张好友券，赶快分享给你的朋友！';
				$picurl='';
