...

{
    ...
    */
...

    {
        ...
        $sql = 'SELECT cat_id,cat_name ,parent_id,cat_img,is_show ' .
...

{
...

    {
        ...
        $child_sql = 'SELECT cat_id, cat_name,cat_img, parent_id, is_show ' .
...

        {
            ...
            if ($row['is_show'])
			   ...
			   $three_arr[$row['cat_id']]['a_img'] = "http://" . $_SERVER['HTTP_HOST']."/".$cat_img;
...

{
    ...
    if (!in_array($type, array('best', 'new', 'hot','tejia','zero')))
...

    {
        ...
        $type_goods['tejia'] = array();

        $type_goods['zero'] = array();
...

        {
            ...
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_tejia, g.is_zero ,g.is_promote, b.brand_name,g.sort_order ' .

               ' FROM ' . $GLOBALS['hhs']->table('goods') . ' AS g ' .

               ' LEFT JOIN ' . $GLOBALS['hhs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .

               ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1 OR g.is_tejia = 1 OR g.is_zero = 1)'.
            ...
            $goods_data['tejia'] = array();

            $goods_data['zero'] = array();
...

            {
...

                {
...

                    {
                        ...
                        $goods_data['tejia'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['sort_order']);
...

                    {
                        ...
                        $goods_data['zero'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['sort_order']);
        ...
        $type2lib = array('best'=>'recommend_best', 'new'=>'recommend_new', 'hot'=>'recommend_hot', 'tejia'=>'recommend_tejia', 'zero'=>'recommend_zero');
        ...
        $type_merge = array_merge($type_array['new'], $type_array['best'], $type_array['hot'], $type_array['tejia'], $type_array['zero']);
...

        {
            ...
            if (in_array($row['goods_id'], $type_array['tejia']))

            {

                $type_goods['tejia'][] = $goods[$idx];

            }

            if (in_array($row['goods_id'], $type_array['zero']))

            {

                $type_goods['zero'][] = $goods[$idx];
...

{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_number,g.goods_thumb, goods_img,  " .
...

    {
		...
		$goods[$idx]['price_discount']    = number_format($row['promote_price']/$row['market_price']*10,1);
...

{
...

    {
        ...
        /* 修正促销价格 */
            ...
            $promote_price = price_format($row['promote_price']);
		...
		$row['goods_attr_num']    = get_goods_attr($row['goods_id']) ? 1 : 0;
...

{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
                ...
                "g.goods_attr_id, g.attr_value, g.attr_price, g.attr_img " .
...

    {
...

        {
			...
			if($row['attr_img'])
			{
				$attr_img = "http://" . $_SERVER['HTTP_HOST']."/".$row['attr_img'];
			}
			else
			{
				$attr_img ='';
...
function spec_price($spec,$is_team = false)

{
...

    {
        ...
        if ($is_team) {
            ...
            $sql = 'SELECT SUM(attr_team_price) AS attr_price FROM ' . $GLOBALS['hhs']->table('goods_attr') . " WHERE $where";
...
function get_typeof_goods($type,$num = 12, $is_best = false,$user_id = null){
    ...
    $where = "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_luck = 0 AND g.is_miao = 0 AND g.is_".$type." = 1 ";
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_number, g.suppliers_id, g.goods_name_style, g.market_price, g.shop_price, g.ts_a, g.ts_b, g.ts_c , ' .
            ...
            "WHERE $where ORDER BY g.sort_order, g.goods_id DESC limit " . $num;
...

{
...

  {
 	 ...
 	 return dds($cat['parent_id'],$cat['cat_id']);
