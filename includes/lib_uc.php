 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: lib_uc.php 17217 2014-05-12 06:29:08Z pangbin $
 */
...
function add_feed($id, $feed_type)
{
    ...
    if ($feed_type == BUY_GOODS)
    {
        ...
        $id = intval($id);
        $order_res = $GLOBALS['db']->getAll("SELECT g.goods_id, g.goods_name, g.goods_sn, g.goods_desc, g.goods_thumb, o.goods_price FROM " . $GLOBALS['hhs']->table('order_goods') . " AS o, " . $GLOBALS['hhs']->table('goods') . " AS g WHERE o.order_id='{$id}' AND o.goods_id=g.goods_id");
        foreach($order_res as $goods_data)
        {
            if(!empty($goods_data['goods_thumb']))
            {
                $url = $GLOBALS['hhs']->url() . $goods_data['goods_thumb'];
            }
            else
            {
                $url = $GLOBALS['hhs']->url() . $GLOBALS['_CFG']['no_picture'];
            }
            $link = $GLOBALS['hhs']->url() . "goods.php?id=" . $goods_data["goods_id"];
