...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    if($order['extension_code']=='team_goods'&&$order['team_status']==1 && $order['is_luck'] == 0 && $order['is_miao'] == 0){
        ...
        if($pay_time && ($pay_time+$GLOBALS['_CFG']['team_suc_time']*24*3600)<gmtime()){
            do_team_refund($order['team_sign']);
            //取消订单
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
            ...
            $sql="select goods_name from ".$GLOBALS['hhs']->table('order_goods')." where order_id=".$order['order_id'];
...
{
    $sql = "SELECT u.bonus_sn, u.order_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date, b.tongyong ".
...
{
    $sql = "SELECT b.free_all,b.type_id,u.bonus_id,u.bonus_sn, u.order_id,u.used_time,u.suppliers_id,b.is_share, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date, b.tongyong ".
        " FROM " .$GLOBALS['hhs']->table('user_bonus'). " AS u ,".
        $GLOBALS['hhs']->table('bonus_type'). " AS b".
        " WHERE u.bonus_type_id = b.type_id AND u.user_id = '" .$user_id. "'";
    ...
    foreach($res as $row){
		...
		if($row['free_all'])
		{
			 $row['goods_name'] = $GLOBALS['db']->getOne("select goods_name from ".$GLOBALS['hhs']->table('goods')." where bonus_free_all='$row[type_id]'");
			 $row['goods_id'] = $GLOBALS['db']->getOne("select goods_id from ".$GLOBALS['hhs']->table('goods')." where bonus_free_all='$row[type_id]'");
			
		}
		
		
        $row['suppliers_name'] =  $row['suppliers_id']?get_suppliers_name($row['suppliers_id']):'自营店';
...
//pangbin 增加抽奖
function get_user_luckdraw($user_id, $num = 10, $start = 0, $where){
	$ntime = gmtime();

	$arr = array();
	$sql="select o.team_status,o.luckdraw_id,o.is_luckdraw,o.order_id,o.order_sn,o.suppliers_id,l.luck_status,l.id,l.start_time,l.end_time from ".$GLOBALS['hhs']->table('order_info') ."as o left join ".$GLOBALS['hhs']->table("luckdraw")." as l on o.luckdraw_id = l.id where o.user_id='$user_id' and o.pay_status >1 ".$where." and o.luckdraw_id > 0 ORDER BY add_time DESC";
	$res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
	while ($row = $GLOBALS['db']->fetchRow($res))
    {	
		$row['goods_list'] = get_order_goods_list($row['order_id']);

			$luckdraw = get_order_luckdraw($row['luckdraw_id']);
			//print_r($luckdraw);


		
			
			if($luckdraw['start_time']<$ntime && $ntime<$luckdraw['end_time'] && $luckdraw['luck_status'] == 0)
		    {
			$row['start'] =1;
		    }
		    elseif($ntime>$luckdraw['end_time'] && $luckdraw['luck_status'] == 0)	
		    {
			$row['start'] =2;
		    }
		    elseif($ntime<$luckdraw['start_time'])	
		    {
			$row['start'] =3;
		    }
			
		$suppliers = $row['suppliers_id'] ? $GLOBALS['db']->getRow('select suppliers_name,supp_logo FROM '.$GLOBALS['hhs']->table('suppliers').' WHERE suppliers_id=' . $row['suppliers_id']):array();
		...
		$arr[] = array(
			...
			'is_luckdraw'     => $row['is_luckdraw'],
			'title'     => $luckdraw['title'],
			'luck_status'     => $luckdraw['luck_status'],
			'content'     => $luckdraw['content'],
...
function get_order_luckdraw($luckdraw_id)
{
    $sql = 'SELECT * FROM '. $GLOBALS['hhs']->table('luckdraw')." WHERE id = ".$luckdraw_id." ORDER BY id DESC";
