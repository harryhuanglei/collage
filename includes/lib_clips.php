 ...
 * $Id: lib_clips.php 17217 2014-05-12 06:29:08Z pangbin $
...
function get_collection_store($user_id, $num = 10, $start = 0)
{
	$sql = "select c.*,s.suppliers_name,s.real_name from ".$GLOBALS['hhs']->table('collect_store')." as c,".$GLOBALS['hhs']->table('suppliers')." as s where s.suppliers_id=c.suppliers_id and c.user_id='$user_id' order by id desc limit $start,$num  ";
	$list = $GLOBALS['db'] -> getAll($sql);
	foreach($list as  $idx=>$value)
	{
		$list[$idx]['add_time']= local_date($GLOBALS['_CFG']['time_format'], $value['add_time']);
		$list[$idx]['logo'] = get_logo($value['suppliers_id']);
	}
	return $list;
...

{
    ...
    $sql = 'SELECT COUNT(*) '.
           ...
           'FROM ' .$GLOBALS['hhs']->table('booking_goods').
...

{
    ...
    $sql = "SELECT * FROM " .$GLOBALS['hhs']->table('feedback');
...

    {
        ...
        //{
            ...
            $sql   = "SELECT user_name, user_email, msg_time, msg_content".
                     ...
                     " FROM " .$GLOBALS['hhs']->table('feedback') .
...

{
    ...
    $sql = "INSERT INTO " . $GLOBALS['hhs']->table('feedback') .
...

{
     ...
     $sql = "DELETE FROM ".$GLOBALS['hhs']->table('tag').
...

{
    ...
    $sql = "SELECT bg.rec_id, bg.goods_id, bg.goods_number, bg.booking_time, bg.dispose_note, g.goods_name ".
           ...
           "FROM " .$GLOBALS['hhs']->table('booking_goods')." AS bg , " .$GLOBALS['hhs']->table('goods')." AS g". " WHERE bg.goods_id = g.goods_id AND bg.user_id = '$user_id' ORDER BY bg.booking_time DESC";
...

{
    ...
    $sql  = "SELECT goods_name FROM " .$GLOBALS['hhs']->table('goods'). " WHERE goods_id = '$goods_id'";
...

{
    ...
    $sql = 'DELETE FROM ' .$GLOBALS['hhs']->table('booking_goods').
...

{
    ...
    $sql = "INSERT INTO " .$GLOBALS['hhs']->table('booking_goods').
...

{
    ...
    $sql = 'INSERT INTO ' .$GLOBALS['hhs']->table('user_account').
...

{
    ...
    $sql = 'INSERT INTO ' .$GLOBALS['hhs']->table('pay_log')." (order_id, order_amount, order_type, is_paid)".
...

{
    ...
    $sql = 'DELETE FROM ' .$GLOBALS['hhs']->table('user_account').
...

{
    ...
    $sql = "SELECT SUM(user_money) FROM " .$GLOBALS['hhs']->table('account_log').

           " WHERE user_id = '$user_id'";

    $sql = "SELECT (user_money - frozen_money) FROM " .$GLOBALS['hhs']->table('users').
           " WHERE user_id = '$user_id'";
...

{
    ...
    $sql = "SELECT pay_points FROM " .$GLOBALS['hhs']->table('users').
           " WHERE user_id = '$user_id'";
...

{
    ...
    $sql = "SELECT user_name,headimgurl,uname,mobile_phone,pay_points, user_money, credit_line, last_login, is_validated FROM " .$GLOBALS['hhs']->table('users'). " WHERE user_id = '$user_id'";
    ...
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['hhs']->table('order_info').
...

{
...

    {
        ...
        $sql = "SELECT COUNT(*) FROM ". $GLOBALS['hhs']->table("tag").
...

        {
            ...
            $sql = "INSERT INTO ".$GLOBALS['hhs']->table("tag")." (user_id, goods_id, tag_words) ".
...

{
...

    {
        ...
        $sql = "SELECT rank_name, special_rank FROM " . $hhs->table('user_rank') . " WHERE rank_id = '$_SESSION[user_rank]'";
...

        {
            ...
            $user_rank = $db->getOne("SELECT rank_points FROM " . $hhs->table('users') . " WHERE user_id = '$_SESSION[user_id]'");

            $sql = "SELECT rank_name,min_points FROM " . $hhs->table('user_rank') . " WHERE min_points > '$user_rank' ORDER BY min_points ASC LIMIT 1";
...

{
    ...
    $sql = "SELECT c.*, g.goods_name AS cmt_name, r.content AS reply_content, r.add_time AS reply_time ".
           ...
           " FROM " . $GLOBALS['hhs']->table('comment') . " AS c ".
...

    {
        ...
        $sql = "SELECT article_id , title FROM " . $GLOBALS['hhs']->table('article') . " WHERE " . db_create_in($to_article, 'article_id');
