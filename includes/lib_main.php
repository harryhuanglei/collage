...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.user_money, u.pay_points,u.devicetoken'.
            ' FROM ' .$GLOBALS['hhs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1,
                        $keywords = '', $brand = 0, $act = '', $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='')
{
    ...
    switch ($app)
    {
        case 'category':
            ...
            break;
		case 'cate':
            $uri_args = array('cid' => $cat, 'sort' => $sort, 'order' => $order, 'display' => $display_type);
            break;
        case 'article_cat':
        ...
        case 'exchange':
            ...
            break;
        case 'rank':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'sort' => $sort, 'order' => $order,'act' => $keywords);
            break;
		case 'store':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'sort' => $sort, 'order' => $order,'act' => $act);
            break;
        case 'tuan':
        case 'mall':
        case 'zero':
...
function assign_template($ctype = '', $catlist = array())
{
  ...
  //  $smarty->assign('catalog_list',  cat_list(0, 0, false, 1, false));
   // $smarty->assign('navigator_list',        get_navigator($ctype, $catlist));  //自定义导航栏
    ...
    $smarty->assign('bonus_ad',     $GLOBALS['_CFG']['bonus_ad']);
    $smarty->assign('subscribe_url',    $GLOBALS['_CFG']['subscribe_url']);
    $smarty->assign('qr_code',    $GLOBALS['_CFG']['qr_code']);
    $smarty->assign('ziti',     $GLOBALS['_CFG']['ziti']);
	$smarty->assign('qiandao',     $GLOBALS['_CFG']['qiandao']);
	$smarty->assign('dbgz',     $GLOBALS['_CFG']['dbgz']);
	$smarty->assign('cat_type',     $GLOBALS['_CFG']['cat_type']);
    $smarty->assign('default_shipping_id',     $GLOBALS['_CFG']['default_shipping_id']);
		$smarty->assign('ziti_start_time',     $GLOBALS['_CFG']['start_time']);
	$smarty->assign('ziti_end_time',     $GLOBALS['_CFG']['end_time']);
	$smarty->assign('open_mobile',     $GLOBALS['_CFG']['open_mobile']);
...
//         $license = '<a href="http://www.baidu.com/license.php?product=hhshop_b2c&url=' . urlencode($host) . '" target="_blank"
// >&nbsp;&nbsp;Licensed</a>';
//         return $license;
//     }
//     else
...
function get_advlist_position_name($position_name,$num)
{
		$redirect_uri="http://" . $_SERVER['HTTP_HOST']."/";
		$arr = array( );
		$sql = "select ap.ad_width,ap.ad_height,ad.ad_id,ad.ad_name,ad.ad_code,ad.media_type,ad.ad_link,ad.ad_id from ".$GLOBALS['hhs']->table( "ad_position" )." as ap left join ".$GLOBALS['hhs']->table( "ad" )." as ad on ad.position_id = ap.position_id where ap.position_name='".$position_name.( "' and ad.enabled=1 order by order_sort limit ".$num );
		$res = $GLOBALS['db']->getAll( $sql );
		foreach ( $res as $idx => $row )
		{
...
function getads($cat,$num,$city_id='')
{
    $pid = $GLOBALS['db']->getOne("SELECT parent_id FROM ".$GLOBALS['hhs']->table('region')." WHERE region_id=".$city_id);
	$where = '';
	if(!empty($city_id) && $city_id != 1)
	{
		$where = " and (city_id =".$city_id." or city_id =".$pid.")";
	}
	$time = gmtime();
	$sql = "SELECT * FROM " . $GLOBALS['hhs']->table('ad') . " where enabled = 1 and position_id=".$cat.$where." ORDER BY ad_id desc limit ".$num;
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
	foreach ($res AS $idx => $row)
	{
 ...
 */
function get_uinfo($uid,$field){
	$sql = "select * from ".$GLOBALS['hhs']->table('users')." where user_id = ".$uid;
	$row = $GLOBALS['db']->getRow($sql);
