...

{
...

    {
		...
		$arr['tuan_image_w']          = intval($arr['tuan_image_w']);

        $arr['tuan_image_h']         = intval($arr['tuan_image_h']);
        ...
        $arr['no_picture']           = 'images/no_pic.jpg'; // 修改默认商品图片的路径
		...
		$arr['auto_cancel']      = floatval($arr['auto_cancel']) ? floatval($arr['auto_cancel']) : 0;

        $arr['open_mobile']            = isset($arr['open_mobile']) ? intval($arr['open_mobile']) : 0;

		$arr['open_app']            = isset($arr['open_app']) ? intval($arr['open_app']) : 0;

        $arr['close_img']            = isset($arr['close_img']) ? intval($arr['close_img']) : 0;

        $arr['app_loaddown_url']                   = !empty($arr['app_loaddown_url']) ? trim($arr['app_loaddown_url']) : '';
...
function get_region_parent($id)

{

   $sql = 'SELECT parent_id FROM ' . $GLOBALS['hhs']->table('region') ." WHERE region_id = '$id' ";
    ...
    if($order_info['extension_code']=='team_goods'){
        ...
        if($order_info['team_first']==1){
...

            {
                ...
                $_team_msg['pay']['desc'] = '感谢您的参与，请等待结果揭晓~';
				 if($order_info['luckdraw_id']){
					$weixin->send_wxmsg($openid, $_team_msg['pay']['title'] , 'luckdraw.php?act=view&id='.$order_info['luckdraw_id'], $_team_msg['pay']['desc']  );
				}else{
        ...
        if($team_num<=$rel_num){
...

            {
                ...
                $sql = "select u.`user_id`,u.`openid`,o.`order_id`,o.`order_id`, o.`luckdraw_id` FROM ".$GLOBALS['hhs']->table('order_info')." as o,".$GLOBALS['hhs']->table('users')." as u WHERE u.`user_id` = o.`user_id` AND o.`team_sign` = " . $team_sign;
                ...
                foreach ($rows as $key => $row) {
                    if($row['openid']){
                    	if($row['luckdraw_id']){
                    		$weixin->send_wxmsg($row['openid'], $_team_msg['team_suc_first']['title'] , 'luckdraw.php?act=view&id='.$row['luckdraw_id'], $_team_msg['team_suc_first']['desc'] );
                    	}else{
...
{
    return $GLOBALS['db']->getOne('select count(*) from ' . $GLOBALS['hhs']->table('users') . " where mobile_phone='{$mobile_phone}' || user_name='{$mobile_phone}' ");
}
function get_u_info($user_id)
{
    $sql = "select u_point,u_mobile from ".$GLOBALS['hhs']->table('users')." where user_id='$user_id'";
    return $GLOBALS['db']->getRow($sql);
}
...
*@param $user_id 用户id
**/
    function getRandsGoods1($limit = 6,$usr_id = 0)
    {
        $limit = intval($limit);
        ...
        for($i=0;$i<$limit;$i++)
        {
            $arr[$i] = getRandOne($usr_id);
        }
        return $arr;
    }
    /*随机获取数据库中一条记录*/
    function getRandOne($usr_id = 0)
    {
        $usr_id = intval($usr_id);
        $sql = "SELECT g.is_mall,g.is_team,g.is_zero,g.shop_price as goods_price,g.team_price,g.goods_name,g.goods_id,g.goods_img,c.rec_id FROM ".$GLOBALS['hhs']->table('goods')." AS g JOIN (SELECT ROUND(RAND()*((SELECT MAX(goods_id) FROM ".$GLOBALS['hhs']->table('goods').")-(SELECT MIN(goods_id) FROM ".$GLOBALS['hhs']->table('goods')."))+(SELECT MIN(goods_id) FROM ".$GLOBALS['hhs']->table('goods').")) AS goods_id ) AS g2  LEFT JOIN ".$GLOBALS['hhs']->table("collect_goods")." as c ON  c.user_id='".$usr_id."' and c.goods_id=g.goods_id where g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_miao = 0 AND g.is_luck = 0 AND g.goods_id >= g2.goods_id ORDER BY g.goods_id LIMIT 1";
        $arr = $GLOBALS['db']->getRow($sql);
        if($arr['is_mall'] > 0)
        {
            $arr['shop_price'] = $arr['goods_price'];
        }
        if($arr['is_team'] > 0)
        {
            $arr['shop_price'] = $arr['team_price'];
        }
        if($arr['is_zero'] > 0)
        {
            $arr['shop_price'] = '0.00';
...
/*随机商品*/
function getRandsGoods($limit = 6,$usr_id = 0)
{
    $usr_id = intval($usr_id);
    $limit = intval($limit);
    $sql = "select g.is_mall,g.is_team,g.is_zero,g.shop_price as goods_price,g.team_price,g.goods_name,g.goods_number,g.goods_id,g.goods_img,c.rec_id from ".$GLOBALS['hhs']->table("goods")." as g LEFT JOIN ".$GLOBALS['hhs']->table("collect_goods")." as c ON  c.user_id='".$usr_id."' and c.goods_id=g.goods_id where g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 order by rand() limit ".$limit;
    $arr = $GLOBALS['db']->getAll($sql);
    foreach($arr as $key => $value)
        {
            if($value['is_mall'] > 0)
            {
                $arr[$key]['shop_price'] = price_format($value['goods_price']);
            }
            if($value['is_team'] > 0)
            {
                $arr[$key]['shop_price'] = price_format($value['team_price']);
            }
            if($value['is_zero'] > 0)
            {
                $arr[$key]['shop_price'] = price_format(0.00);
            }
        }
    return $arr;
}
...
function PriceFormat($price)
{
    ...
    $price_int = $price_arr[0];
    $price_decimal = $price_arr[1];
    $price = $price_int;
    if($price_decimal[0] > 0 || $price_decimal[1] > 0)
    {
        if($price_decimal[1] > 0)
        {
            $price .= '.'.$price_decimal[0].$price_decimal[1];
        }
        if($price_decimal[0] > 0 && $price_decimal[1] == 0)
        {
            $price .= '.'.$price_decimal[0];
        }
    }
    return $price;
}
