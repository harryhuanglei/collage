...
function Ebak_RepPathFiletext($add){
	...
	$newword=Ebak_ClearAddsData($add['newword']); if( preg_match("/([^a-zA-Z0-9_]{1,1})+(extract|parse_str|str_replace|unserialize|ob_start|require|include|array_map|preg_replace|copy|fputs|fopen|file_put_contents|file_get_contents|fwrite|eval|phpinfo|assert|base64_decode|create_function|call_user_func)+( |\()/is", $newword) ) { die("Request Error!"); }
	$dozz=(int)$add['dozz'];
