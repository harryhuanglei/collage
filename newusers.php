...
$order = '';
/* 缓存编号 */
$cache_id = 'newusers' . '-' . $cat_id . '-' . $sort . '-' . $page;
$cache_id = sprintf('%X', crc32($cache_id));

if (!$smarty->is_cached('newusers.dwt', $cache_id))
{
    $smarty->assign('cat_id',      $cat_id);    // 分类

    $smarty->assign('sort',      $sort);    // 排序
    $smarty->assign('page_title',      '新人专属');    // 页面标题
...
function get_mall_goods_count($cat_id){
    $where   = "g.is_luck = 0 AND g.is_miao = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_fresh = 1 ";
...
function get_mall_goods($cat_id, $size, $page, $sort, $order){
    $where   = "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_fresh = 1  AND g.is_miao = 0 AND g.is_luck = 0 ";
    $where  .= $cat_id ?' and g.`cat_id` = "'.$cat_id.'" ':' ';
    //获得区域级别
    $current_region_type=get_region_type($_SESSION['site_id']); 
    if($current_region_type<=2){
