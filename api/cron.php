...
function get_cron_info()
{
    ...
    $sql   = "SELECT * FROM " . $GLOBALS['hhs']->table('crons') . " WHERE enable = 1 AND nextime < $GLOBALS[timestamp]";
...
function write_error_arr($err_arr)
{
    if (!empty($err_arr))
    {
        ...
        if ($query)
        {
            $sql = "INSERT INTO " . $GLOBALS['hhs']->table('error_log') . "(info, file, time) VALUES " . $query;
