...
$openid = isset($_REQUEST['op']) ? trim($_REQUEST['op']) : 0;
$money = isset($_REQUEST['m']) ? trim($_REQUEST['m']) : 0;
...
if($ua==1){//微信上打开
    ...
    $smarty->display("toalipay_chong.dwt");
}else{//其他浏览器上打开
    ...
    $pay_online = $pay_obj->get_code($pay_order, unserialize_config($payment['pay_config']),'chong');
