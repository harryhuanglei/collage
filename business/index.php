...
$smarty->assign('business_logo', $_CFG['business_logo']);
...
{
    if(!in_array($action, array('login' ,'act_login_sub')) && $action !='forget_pwd' && $action !='reset_pwd')
    {
        $action = 'login';
        $op = 'login';
    }
...
{
    if(checkmobile()){
        include_once(ROOT_PATH . 'business/m_'.$op.'.php');
	}else{
