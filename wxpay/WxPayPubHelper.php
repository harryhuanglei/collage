...

{
	...
	//	echo '【string1】'.$String.'</br>';exit;
		...
		//签名步骤二：在string后加入KEY
        if($this->appwxpay_key){
            $String = $String."&key=".$this->appwxpay_key;
        }
...
public function filepost($url, $post = null) {       
	...
	*/
	...
	function apppostXmlSSLCurl($xml,$url,$second=30,$ssl)

	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT,$ssl['APPSSLCERT_PATH'] );//WxPayConf_pub::

		//默认格式为PEM，可以注释

		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');

		curl_setopt($ch,CURLOPT_SSLKEY,$ssl['APPSSLKEY_PATH'] );//WxPayConf_pub::
...

{
	...
	function apppostXmlSSL($ssl)

	{	
	    
	    $xml = $this->createXml();

		$this->response = $this->apppostXmlSSLCurl($xml,$this->url,$this->curl_timeout,$ssl);
...
class UnifiedOrderapp_pub extends Wxpay_client_pub
{	
	...
	function createXml()
	{
        ...
        $this->total_fee = $this->parameters["total_fee"];
        $this->trade_type = $this->parameters["trade_type"];
        $this->nonce_str = $this->createNoncestr();
        $this->spbill_create_ip = $_SERVER['REMOTE_ADDR'];
        $this->params['appid'] = $this->app_wxpay_app_id;
        $this->params['mch_id'] = $this->app_wxpay_mchid;
		if($this->parameters["attach"])
		{
			$this->params['attach'] = $this->parameters["attach"];
		}
        ...
        $this->params['notify_url'] = $this->parameters['notify_url'];
        $this->params['trade_type'] = $this->trade_type;
	
		
		
          //获取签名数据
        $this->sign = $this->getagensign( $this->params );
		...
		//print_r($result);exit;
		//echo $this->out_trade_no."<br>";
		
        if( !empty($result['result_code']) && !empty($result['err_code']) ){
          //  $result['err_msg'] = $this->errorMessage( $result['err_code'] );
        }
		//echo "<pre>";
...
//
//			}
//		   	$this->parameters["appid"] = $this->app_wxpay_app_id;//公众账号ID
//		   	$this->parameters["mch_id"] = $this->app_wxpay_mchid;//商户号
//		   	$this->parameters["spbill_create_ip"] = $_SERVER['REMOTE_ADDR'];//终端ip	    
//		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
//		
//			
//		
//		    $this->parameters["sign"] = $this->appgetSign($this->parameters,$this->appwxpay_key);//签名
//				//echo "<pre>";
//			//var_dump($this->parameters);exit;
//			
//			
//			
//			//echo  $this->arrayToXml($this->parameters);exit;
//		    return  $this->arrayToXml($this->parameters);
...
//		}
     ...
     function getagensign( $params ){
	  ...
	  function getagensigntwo( $params ){
        //签名步骤一：按字典序排序数组参数
        ksort($params);
        $string = $this->ToUrlParams($params);
        //签名步骤二：在string后加入KEY
...
//	function getagensign($Parameters)
//	{
//		
//	
//		foreach ($Obj as $k => $v)
...
//		}
	...
	function apppostxml()
	{
		$result = $this->postXmlCurl($this->createXml(),$this->url,30);
...
class Refundapp_pub extends Wxpay_client_pub
...
//
		  ...
		  // 	$this->parameters["mch_id"] = $this->app_wxpay_mchid;//商户号
		    ...
		    $this->parameters["sign"] = $this->appgetSign($this->parameters);//签名
	...
	function getResult($ssl) 
	{	

		$this->apppostXmlSSL($ssl);
		$this->result = $this->xmlToArray($this->response);
...

{
	...
	function appcheckSign()
	{
		...
		$sign = $this->appgetSign($tmpData);//本地签名
